...
| 导出（正式） | **训练机 `play.py --headless` 产出**：`…/2026-09-17_17-19-04/exported/policy.pt`，sha256 `18e222c0d745e0ae…`，769312 B（含 `policy.onnx`）；已按字节复制到 `amp/policy.pt`（2026-09-18 用户执行） |
| 导出（复刻，已退居备用） | 我先前用 Isaac Lab `_TorchPolicyExporter` 等价复刻的 `0e5bd661b1591a87…`、770931 B；两者数值完全相同（见 9.5），文件长度差异只是序列化元数据 |
| 前一代 | 48 维 `model_9000.pt` 的导出 `e5a8bfec742038e0…`，可从 git 历史取回 |
...
**协议（重要）**：**每条速度指令用一条全新的 Gazebo + `rl_sim` 栈**，窗口 = 进入策略后 3 s 起、24 s。
不要在同一条栈里连着跑多个 `vx`：评测脚本开头会按 6 s 的 A（起身），而栈已经在 AMP 里时再按 A
会**重新触发 GetUp**，测量窗口却仍从本次按键算起 ⇒ 均值被起身段稀释。实测代价：`vx=0.5` 在
"同栈连跑"协议下量到 0.151（三连跑那次）甚至 0.039 m/s，而**每条新栈**量到 0.352–0.380 m/s。
为此脚本加了 `--no-getup`（仅在确认已在策略状态时用）。

**正式导出件（`play.py`，sha256 `18e222c0d745e0ae…`）每条新栈一次的实测**：

| vx 命令 | z_mean（min–max） | roll / pitch 范围 | yaw 漂移 | dx / 实测速度 | 抖动 hip/thigh/shank | 周期 / 强度 / FL-FR 相位 |
|---|---|---|---|---|---|---|
| **0.0** | 0.272（0.263–0.287） | 3.54° / 2.53° | −3.92°/s | 0.70 m（0.047 m/s） | 35.7 / 39.8 / 80.1 | 0.460 s / 0.46 / −96.5° |
| **0.5** | 0.281（0.266–0.298） | 6.75° / 5.07° | +2.72°/s | 5.70 m（**0.380 m/s = 76%**） | 46.0 / 71.2 / 124.9 | 0.427 s / 0.18 / 123.3° |
| **1.0** | 0.286（0.266–0.312） | 9.18° / 7.30° | +1.14°/s | 14.57 m（**0.972 m/s = 97%**） | 65.2 / 116.5 / 220.9 | 0.294 s / 0.20 / −161.9° |

对照：同一脚本下键 1 的参考 `base_move/policy_flat.pt` 在 `vx=0.5` 是 **0.428 m/s（86%）**、
z 0.288、roll 2.5°、thigh 抖动 45.3、周期强度 **0.95**、相位 175°；旧的 48 维 `model_9000.pt`
在 `vx=0.5` 只有 0.057 m/s、z 0.182。

**读法**：

1. **三项契约全过**：① 正式导出件、我的等价复刻、checkpoint 的 actor 三者两两 `max|diff| = 0.0`
   （128 组随机 45 维输入，见 9.5 节）；② 两个文件都能被部署自己的 libtorch 2.3.0 加载，同探针输出
   与 Python 一致到 1e-6；③ `rl_sim` 打印 `Successfully loaded Torch model: …/amp/policy.pt`、FSM 进入
   `RLFSMStateAMPLocomotion`，45 维观测下站姿与指令响应都正常。⇒ **"等价复刻导出的结论"已被正式
   导出件取代，两者行为一致**（复刻件在 `vx=0.5` 同协议下量到 0.352 m/s，与正式件的 0.380 属
   同一会话间波动量级）。
2. **速度跟踪是通的、而且越大指令越好**：0.5 → 0.380 m/s（76%），1.0 → 0.972 m/s（**97%**）；
   旧 48 维那份在 0.5 只有 0.057。高度随速度保持（0.272→0.286 m），没有为了跟速而贴地。
3. **步态仍不干净**：周期强度只有 0.18–0.20（PPO 0.95）、FL-FR 相位 123°/−162°（trot 应为 ≈±180°）、
   抖动随速度上升（shank 80→125→221 rad/s²，PPO 在 0.5 只有 45）。`vx=0` 也不安静
   （0.047 m/s 漂移、yaw −3.9°/s、shank 抖动 80）。
   ⇒ **接口与速度跟随已达标，剩下的是步态质量**，属训练侧（轮数、奖励结构、低姿态终止）问题；
   下一步用训练机的 `eval_gait.py` 看足端占空比/落地相位，与本表的关节侧指标互补。
...
**三条契约已对正式导出件执行完毕（2026-09-18，本机）**：

| 契约 | 做法 | 结果 |
|---|---|---|
| ① 与 checkpoint 的 actor 数值一致 | 128 组随机 45 维输入，比较 `exported/policy.pt`、我的复刻件、`model_5000.pt` 的 actor | **三者两两 `max diff = 0.0`**（Python torch 2.7 CPU） |
| ② 部署 libtorch 能加载 | 用部署自带的 **libtorch 2.3.0+cpu**（`torch::jit::load`）加载两个文件，喂同一确定性探针 | 都 `LOADED`，12 维输出与 Python 侧一致到 1e-6（float32 打印精度） |
| ③ 部署行为一致 | Gazebo `--key 3`，每条命令一条新栈 | 见 9.2；正式件 `vx=0.5` 0.380 m/s，复刻件同协议 0.352 m/s（会话间波动量级） |

正式导出件与复刻件**文件长度不同**（769312 对 770931 B），只是 TorchScript 序列化的元数据差异，
数值与部署行为相同。
...
### 9.6 速度扫描 + 与键 1 参考 PPO 的同协议对比（2026-09-18 补）

用户观察：「新的 AMP 策略在速度较高的情况下跑起来还行」。为此在**每条命令一条全新栈**的协议下扫了
AMP（键 3）的速度点，并对键 1 参考 `base_move/policy_flat.pt` 做了同样的测量。复现脚手架
`imgo2_deploy/build/run_gz_test.sh` 现在支持 `VX_LIST` / `KEY` / `TAG` 三个环境变量，并把过滤后的
`rl_sim` 日志存成 `build/gztest/rlsim_<TAG>_vx<v>.log`（`build/` 被忽略，不入库）。

| 策略 | vx | z_mean | roll | pitch | yaw 漂 | 实测 m/s | 跟速 | hip/thigh/shank 抖动 | 周期 | 强度 | FL-FR 相位 |
|---|---|---|---|---|---|---|---|---|---|---|---|
| AMP | 0.0 | 0.272 | 3.54° | 2.53° | −3.92°/s | 0.047 | — | 35.7 / 39.8 / 80.1 | 0.460 s | 0.46 | −96.5° |
| AMP | 0.5 | 0.281 | 6.75° | 5.07° | +2.72°/s | 0.380 | 76% | 46.0 / 71.2 / 124.9 | 0.427 s | 0.18 | 123.3° |
| AMP | 0.75 | 0.284 | 7.38° | 6.13° | +2.51°/s | 0.632 | 84% | 53.8 / 95.5 / 167.2 | 0.296 s | 0.17 | −141.0° |
| AMP | 1.0 | 0.286 | 9.18° | 7.30° | +1.14°/s | 0.972 | **97%** | 65.2 / 116.5 / 220.9 | 0.294 s | 0.20 | −161.9° |
| AMP | 1.2 | 0.288 | 8.87° | 6.45° | +1.13°/s | 1.196 | **99.7%** | 68.2 / 133.7 / 259.8 | 0.213 s | 0.36 | **−180.8°** |
| AMP | 1.5 | 0.289 | 8.76° | 8.08° | +0.60°/s | 1.492 | **99.4%** | 75.9 / 156.8 / 315.8 | 0.208 s | 0.55 | 156.4° |
| PPO 参考 | 0.5 | 0.288 | 2.50° | 1.42° | −0.38°/s | 0.465 | 93% | 23.8 / 45.5 / 93.1 | 0.430 s | **0.96** | −184.6° |
| PPO 参考 | 1.0 | 0.286 | 3.07° | 3.02° | +0.29°/s | 0.945 | 94.5% | 37.1 / 83.7 / 144.9 | 0.369 s | **0.97** | 176.1° |
| PPO 参考 | 1.2 | 0.284 | 3.39° | 3.40° | +0.61°/s | 1.144 | 95.4% | 43.2 / 104.3 / 172.4 | 0.357 s | **0.97** | 175.0° |

（AMP 0.5 那行是复刻件同协议的测量；正式件那一次为 0.380 m/s、跟速 76%，两者同量级。）

**读法**：

1. **用户的观察被数据证实**：AMP 的跟速随指令**单调上升**（76% → 84% → 97% → 99.7% → 99.4%），
   在 **≥1.0 m/s 时反超参考 PPO**（97.2% 对 94.5%），高度全程平（0.272→0.289 m），偏航漂移也随
   速度减小（2.7→0.6°/s）。1.5 m/s 已经**超出参考动作数据能演示的上限**（录制里最快 0.83 m/s，
   见 [AMP 对照报告](amp_standstill_diagnosis_2026-09-17.md) 11.4 节）却仍保持 99% 跟速。
2. **但姿态与步态质量明显不如参考**：roll 8.8–9.2° 对 3.1–3.4°、pitch 6.5–8.1° 对 3.0–3.4°
   （约 2–3 倍），抖动同速度下高 1.3–1.5 倍，而**周期强度只有 0.17–0.55，参考是 0.96–0.97**。
   1.2 m/s 处相位 −180.8° 说明它已经是**对角步态**，但强度低意味着每步形状重复得不好
   ——视觉上"还行"，实际是快速倒腿凑出正确的平均速度。
3. **低速是它的弱项**：0.5 只有 70–76%，`vx=0` 时还有 0.047 m/s 漂移与 −3.9°/s 偏航，抖动也
   不比行走时低多少；而参考 PPO 在 0.5 是 93%、强度 0.96、`vx=0` 完全静止。
   ⇒ 实用路由建议：**≥1.0 m/s 用键 3（AMP），低速/站立用键 1（PPO）**；要它自己在低速也干净，
   需要训练侧继续改（这正是 AMP-07 未做的低姿态终止、σ、指令范围与轮数）。
4. AMP 表现最好的是 **~1.2 m/s**（相位 −180.8°、强度 0.36、跟速 99.7%、高度 0.288）。

**顺带修掉一个真 bug（我自己引入的）**：`policy/imgo2/ppo/config.yaml` 里顶层 key
`imgo2/ppo:` **重复写了一行**（提交 `ee0a669` 加注释头时把 key 行也抄了一遍）。yaml-cpp 取的是
**第一个**（空的）节点 ⇒ `model_name` 读到空串 ⇒

```
[ERROR] InitRL() failed: Unknown model file extension: .  Supported: .pt, .pth, .onnx
```

`RLFSMStatePPOLocomotion::Enter()` 捕获后 `RequestStateChange("RLFSMStatePassive")`，于是**键 1
一直在静默失效**：FSM 走 Passive→GetUp→PPOLocomotion→Passive，机器人无扭矩趴在地上
（实测 z 0.0922 m、roll 0.003°、抖动 5–9）。删掉重复行后键 1 立即恢复：`vx=0.5` 0.465 m/s（93%）、
...
### 9.7 `model_24500`（24500 轮）确认：低速与站立已达标（2026-09-18 补）

用户报告「24500 轮这份表现很好，基本上小速度可以跟随了」。按 9.2 的协议（**每条命令一条全新的
Gazebo + `rl_sim` 栈**）复测，并与 9.6 的 5000 轮结果逐点对照。

**被测对象**：`policy/imgo2/amp/model_24500.pt`，sha256 `cc8ee22ace4f5240…`；
部署件 `policy/imgo2/amp/policy.pt`，sha256 `cba59d44e387834e…`（769312 B）。
契约① 导出件 vs checkpoint actor：128 组随机 45 维输入 **max diff = 0.0**；
契约② 部署自带 libtorch 2.3.0 加载成功、同探针输出与 Python 一致到 1e-6。
**顺带确认了 AMP-08 的修复**：这份 checkpoint 的 `iter` 字段是 **24500**（不再是 0）——
这是训练侧「修复后尚未产生新 checkpoint」那条未验证项的第一个实证。

| vx | 5000 轮 跟速 | **24500 轮 跟速** | 5000 roll | **24500 roll** | 5000 周期强度 | **24500 周期强度** | 5000 相位 | **24500 相位** |
|---|---|---|---|---|---|---|---|---|
| 0.0 | 0.047 m/s 漂 | **0.001 m/s** | 3.54° | **2.36°** | 0.46 | 0.93 | −96.5° | — |
| 0.2 | 未测 | **93.4%** | — | 3.78° | — | 0.23 | — | −91.3° |
| 0.3 | 未测 | **97.9%** | — | 3.50° | — | 0.78 | — | 147.9° |
| 0.5 | 76.0% | **95.9%** | 6.75° | **3.88°** | 0.18 | **0.71** | 123.3° | **174.4°** |
| 0.75 | 84.3% | **98.7%** | 7.38° | 5.49° | 0.17 | **0.79** | −141.0° | **176.9°** |
| 1.0 | 97.2% | **99.8%** | 9.18° | **5.88°** | 0.20 | **0.83** | −161.9° | **−176.7°** |
| 1.2 | 99.7% | 99.8% | 8.87° | **6.03°** | 0.36 | **0.78** | −180.8° | −183.3° |
| 1.5 | 99.4% | 98.5% | 8.76° | 8.70° | 0.55 | **0.80** | 156.4° | **−187.8°** |
| （1.5 的 z_mean：24500 为 0.300 m，5000 为 0.289 m；全程 0.277–0.300 m 不塌） | | | | | | | | |

**抖动（hip/thigh/shank，`mean|Δdq|/dt`）**：
5000 轮 `0.0 → 35.7/39.8/80.1`、`0.5 → 46.0/71.2/124.9`、`1.0 → 65.2/116.5/220.9`；
24500 轮 `0.0 → **8.6/6.1/9.3**`、`0.5 → 26.5/61.5/109.8`、`1.0 → 47.9/107.3/255.6`。

**读法**：

1. **低速确认成立**：0.2 / 0.3 / 0.5 分别 **93.4% / 97.9% / 95.9%**（5000 轮在 0.5 只有 76%），
   整条曲线除 0.2 外都 ≥95.9%，高度全程 0.277–0.300 m。
2. **站立从"抖着漂"变成"真静止"**：`vx=0` 漂移 0.047 → **0.001 m/s**、yaw −3.9 → −1.9°/s、
   抖动降 4–9 倍（35.7/39.8/80.1 → 8.6/6.1/9.3）。说明 5000 轮时那个"站着不动"的退化解已消失。
   （该行"周期/强度"列在 `vx=0` 时信号近乎常数，不具判读意义。）
3. **步态由"高频乱倒腿"变成"干净对角步"**：周期强度 0.17–0.20 → **0.71–0.83**（参考 PPO 是 0.96–0.97，
   已同量级），FL-FR 相位在 0.3–1.5 m/s 全落在 **174°–188°**（trot 应 ±180°），
   连 5000 轮偏离很大的 0.5（123°）与 1.5（156°）都归位。
4. **姿态同步改善**：同速度 roll 6.8–9.2° → 3.5–6.0°、pitch 也降，yaw 漂移全程 ≤1°/s。
5. **两项仍未达标（不夸大）**：
   - **步频仍约为录制参考的 2.5 倍**：周期 0.208–0.365 s 对参考的 0.600 s / 1.67 Hz
     （见 `docs/gait_reference_baseline.json`）。即"这是更快的干净步"，还不是"像真机录的那样走"。
   - **抖动绝对值随速度上升**（shank 57 → 110 → 256 → 314 → 412），1.0 m/s 以上比 5000 轮略高；
     但同期周期从 0.294 缩到 0.219 s（步频更快），jerk 代理指标本就随步频上升，
     配合"强度 0.8 + 相位正确"看，属"更快的规整步"而非重新变乱。
   - 极低速 0.2 m/s 的周期强度只有 0.23（0.3 是 0.78），规律性偏弱，但跟速已 93%。
