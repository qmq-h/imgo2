...
- **P1/P2 验收（2026-09-20 已在训练机完成并通过）**：用户明确面向训练机的 Isaac Lab，本机不启动或安装该运行环境。**2026-09-20 更新**：训练机（RTX 4090 / Isaac Sim 5.0 / IsaacLab 0.45.9）按第 6 节完成 P1 落地、P2 单工况、`--dt 0.0025` 复核与 8 case 阻力扫描，全部通过；离线 5 项检查两台机器一致通过。P1 四轮接触、静止高度 0.150000 m、质量 10 kg、刚度/阻尼为零；P2 停止距离随 b 单调下降（0.5 m/s：1.034→0.516→0.257 m；1.0 m/s：2.111→1.053→0.524 m），六个已停止工况与解析黏性模型吻合 1% 以内，`calibration.json` 候选 b 为 `[0.016, 0.032]`。操作前提：**Isaac Sim 须串行执行**，并发实例会争锁导致中断。过程、环境依据与两个遗留问题（CART-02 初始化失败退出码为 0、CART-03 离线重算报错）见 [P1/P2 检查与验收记录](cart_p1_p2_checks_2026-09-20.md)。仍待验证的是**训练机物理验收之外**的部分：P3 及以后（绳索张力、上层策略）。
