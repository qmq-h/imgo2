...
## 16. rlamp 首轮训练的中途观察（2026-09-18 21:30，跑在 4391/10000 轮）

运行：`logs/amp_rsl_rl/base_move_amp_rlamp/2026-09-18_20-18-37`（4096 环境、`max_iterations=10000`、
约 1.35 s/轮）。读数用新增的 `imgo2_rl/scripts/tools/read_tfevents.py`（纯标准库，可在训练中直接读，
尾部半帧会被丢弃）。

| 迭代 | 高度 m | 贴地率(<0.20 m) | **摆动时长 s** | **腾空比例** | 基座触地终止 | err_xy | err_yaw | 任务奖励/步 | d_expert | d_policy |
|---|---|---|---|---|---|---|---|---|---|---|
| 500 | 0.2281 | 51.6% | 0.0453 | 0.390 | 16.6% | 1.539 | 1.429 | 0.348 | +0.490 | −0.650 |
| 1000 | 0.3038 | 0.19% | 0.0356 | 0.114 | 13.2% | 1.872 | 1.493 | 0.375 | +0.484 | −0.484 |
| 2000 | 0.3056 | 0.09% | 0.1135 | 0.173 | 10.2% | 1.597 | 1.547 | 0.366 | +0.459 | −0.458 |
| 3000 | 0.3073 | 0.06% | 0.1673 | 0.201 | 8.1% | 1.559 | 1.360 | 0.387 | +0.457 | −0.457 |
| 4000 | 0.3040 | 0.07% | 0.1691 | 0.248 | 7.9% | 1.385 | 1.452 | 0.451 | +0.456 | −0.452 |

**① 最关键的变化是步频（这是 24500 那版始终没解决的缺陷）**：`AMP/mean_last_air_time_s`
（最近一次摆动时长，与 `eval_gait.py` 的 `air_time_mean_s` 同口径）从 500 轮的 **0.045 s** 长到
4000 轮的 **0.169 s**；`AMP/mean_air_time_fraction`（此刻腾空的足比例）0.248。
周期可由两者反推：`周期 ≈ 摆动时长 ÷ 腾空比例` = 0.169 / 0.248 ≈ **0.68 s**，
与录制参考的 **0.600 s**（`docs/gait_reference_baseline.json`）同量级；
而 24500 那版实测周期是 **0.193 s**（快 3.1 倍）。⇒ 单看训练日志，"高频倒腿"这个
唯一实质缺陷在这一轮被明显纠正了（**推断，待 `eval_gait.py` 确认**，见下）。

**② 代价是跟速变差（符合配方预期，不是 bug）**：同迭代点（4000）对比 24500 那轮
（`base_move_amp/2026-09-17_21-25-57`）：

| 4000 轮 | 本轮 rlamp（2 项任务、coef2/lerp0.3） | 24500 那轮（6 项任务） |
|---|---|---|
| `error_vel_xy` | 1.385 | 0.654 |
| `error_vel_yaw` | 1.452 | 0.565 |
| 高度 / 贴地率 | 0.304 / 0.07% | 0.306 / 0.07% |
| 基座触地终止 | 7.9% | 3.7% |
| d_expert / d_policy | +0.456 / −0.452 | +0.638 / −0.637 |

两点必须说清楚，别把它当成"rlamp 更差"：① **指令范围不同**（rlamp 按参考放成
x∈[−1, 2.0]、y±0.3，旧轮是 x∈[−1, 1.5]、y±1.0）；② **任务奖励权重不同**（rlamp 每步
1.0 / 0.3333，旧轮 4.0 / 2.0）。所以 err 只能看**趋势**不能跨配方比绝对值。

**③ 风格项现在是真正的主导项**：`d = −0.448` ⇒ 风格原始分 `2·clamp(1−(d−1)²/4) = 0.946`，
按 `(1−lerp)` 加权后 **0.66/步**；任务项 `0.3 × 0.4507 = 0.135/步` ⇒ **风格占 83%**
（24500 那轮是 28%）。这与 §3.3「判别器在策略点的局部梯度只有 8e-5」并不矛盾：
梯度小意味着"加权重不会给方向"，但把风格占比从 28% 提到 83% 之后，策略确实被推到了
数据的**时间尺度**上（摆动时长 ×3.7）。⇒ §1 里"不是风格占比不够，是风格项没有梯度"这句
需要**部分修正**：占比不是唯一原因，但**确实起了作用**；真正的判据是换来的步态是否更像录制数据。

**④ 下一步（需 GPU 终端，约 2 分钟）**：用与 24500 那次**同口径**的回放确认步态指标：

```bash
# 在 /root/Desktop/Imgo2 下；play 配置把指令固定为 vx=1.0 / vy=0 / yaw=0
/opt/conda/envs/isaaclab/bin/python imgo2_rl/scripts/tools/eval_gait.py \
  --task=Imgo2-basemove-flat-amp-rlamp-play \
  --checkpoint=logs/amp_rsl_rl/base_move_amp_rlamp/2026-09-18_20-18-37/model_4000.pt \
  --num_envs=8 --steps=1000 --out ../docs/gait_eval_rlamp_4000.json --headless
```

要看的项（对照 `docs/gait_reference_baseline.json` 与 `docs/gait_eval_amp_24500_2026-09-18.md`）：
