...
### 29.23 新 run `cmoe_C_77dim_gait` 首查（376 轮）：配方全在、**没有绕过迹象**，但两点要盯

运行 `logs/cmoe/base_move_cmoe_rough/2026-09-24_19-44-08_cmoe_C_77dim_gait`（19:44 起，4.15 s/轮 ⇒ 60000 轮 ≈ 69 h）。

#### 29.23.1 配方核对（从 run 自己的 `params/env.yaml` + `model_0.pt`）

| 项 | 实测 |
|---|---|
| 契约 | `terrain_estimator=(128,77)`、`expert actor=(512,157)`、`critic=(512,125)`、`iter=0`（从零） |
| 命令 | `forward_only_terrain_names` = **全部 8 类地形** |
| 步态 | `feet_gait 1.0 / TrotWithoutGapReward`、`feet_air_time_variance −8.0`（free=boxes,gap）、`lin_pos_y −0.4`、`yaw_abs −0.2`（`terrain_names=()` 全局）、`track_world_vel_xy_exp 5.0` |
| 课程 | `tracking_term_name=track_world_vel_xy_exp`、`tracking_move_up=0.8`、`tracking_move_down=0.35` |

#### 29.23.2 健康（376 轮 vs 上一个 run 同龄）

| | C（新） | B（旧） |
|---|---|---|
| `mean_episode_length` | **957** | 879 |
| `illegal_contact` | **15.3%** | 17.1% |
| `Policy/mean_noise_std` | 0.874 | 0.876 |
| `mean_reward` | 66.4 | 55.1（配方/地形不同，不可比） |
| `Loss/learning_rate` | 1.73e-4 | 贴 1e-5 |
| `Perf/total_fps` | 2.47e4 | 2.17e4 |

#### 29.23.3 **绕过：没有**（三个独立探针 + 结构保证）

用日志反解（`|Δy| = |Episode_Reward/lin_pos_y| / 0.4`；`|yaw| = |Episode_Reward/yaw_abs| / 0.2`）：

| iter | 平均 `\|Δy\|` | 平均 `\|yaw\|` | `distance/progress` |
|---:|---:|---:|---:|
| 50 | 0.48 m | **26.1°** | **1.640** |
| 150 | 0.48 m | 8.4° | 1.019 |
| 250 | 0.57 m | 5.7° | 1.012 |
| 376 | **0.52 m** | **4.8°** | **1.008** |

⇒ 第 50 轮时它**确实在拐着走**（偏航 26°、比前向进度多走 **64%** 路程＝横移/绕行），到 150 轮已被压成**直线前进**
（4.8°、只多 0.8%）。`terrain_out_of_bounds` 全程 **0**。结构上还有三重保证：晋级只看 **Δx**（横移不晋级）、
`lin_pos_y` **全局**、速度目标在**世界系 +x**。
残留理论空间：横移 **2 m** 才能换到相邻列，而实测平均 `|Δy|` 0.52 m（出生随机本身就带 ±0.5 m ⇒ 净漂移约 0.3 m）。

#### 29.23.4 要盯的两点

**(a) 课程门控把"障碍列"卡在最低难度。**
`tracking_pass_frac` **恰好等于** `move_up_frac`（都是 **0.612**）⇒ 晋级实际由"`track > 0.8`"**单条件**决定
（进度早就够：`progress_mean` 11.7 m ≫ 4 m）；`frozen_frac` **0.309**（卡在 0.35–0.80 之间）。
逐列等级：`flat 4.56 ／ slope_inv 3.73 ／ slope 2.82 ／ gap 1.67 ／ rough 1.04 ／ stairs 1.02 ／
stairs_inv 0.12 ／ **boxes 0.03**` ⇒ **无障碍列在涨、障碍列钉在底部**（boxes 只有 9.1 cm）。
**若几千轮后仍如此**，说明 0.80 对"边爬边跟速"太严。两种修法（**都需重启才生效**）：
① 按地形分档：障碍列用更松的阈值（如 0.60）；② 或改成"`track > 0.8` **或** 前向进度 > 1.5×tile(12 m)"，
让一次漂亮的穿越也能晋级。

**(b) `feet_gait` 的动态范围被 `max_err` 夹窄（本次新发现）。**
`GaitReward._sync/_async_reward_func` 里 `se = clip(Δ², max=max_err²=0.04)`，`std=sqrt(0.5)=0.7071`
⇒ 同步核下界 `exp(−0.08/0.7071)=0.893`、反相核下界 `0.893` ⇒ **乘积的下界是 `0.893²·0.893⁴ = 0.508`**，
上界 1 ⇒ **trot 与 bound/pronk 的奖励差最多只有 0.49/s**（同期 `track_world_vel` ≈3.9/s）⇒ 相位项的
"选择性"只有约 **12%**。实测 `Episode_Reward/feet_gait = 0.565`（掩码后；除掉 35% 豁免列 ≈ **0.87**）
⇒ 落在 [0.508, 1] 的中上段（该指标分辨力有限，不能据此断言已是 trot）。
**若 play 仍见 bound/pronk，正确调法不是加权**（加权同时抬高那条 0.5 的"保底奖励"），而是
**放宽 `max_err`（0.2 → 0.5）并减小 `std`（0.7071 → 0.2）** 把 kernel 的脊变陡（下界降到 ~0.001），
把区分度拉满。

#### 29.23.5 建议

**先不重启**（长训才跑 26 分钟，两点都是观察项）。过 2–4 小时（≈2000–3500 轮）复查：
`level_boxes`/`level_stairs_inv` 有没有起来、`feet_gait` 有没有继续爬、`mean_noise_std` 有没有抬头，
再决定是否重启并改阈值。

### 29.24 用户反馈「反楼梯不是很好」＝**卡在 frozen 带**，已改按地形分档阈值（2026-09-24 晚）

#### 29.24.1 现象（447 轮，比 376 轮更清楚）

| 列 | 376 轮 | 447 轮 | |
|---|---:|---:|---|
| `flat` | 4.56 | **6.01** | ↑ |
| `hf_pyramid_slope_inv` | 3.73 | **4.85** | ↑ |
| `hf_pyramid_slope` | 2.82 | 3.54 | ↑ |
| `gap` | 1.67 | **2.63** | ↑（用户也确认在变好） |
| `pyramid_stairs` | 1.02 | **1.95** | ↑ |
| `random_rough` | 1.04 | 1.61 | ↑ |
| **`pyramid_stairs_inv`** | 0.12 | **0.12** | **完全不动** |
| **`boxes`** | 0.03 | **0.10** | 几乎不动 |

#### 29.24.2 根因：不是"过不去"，是**卡在 frozen 带**

三条日志互证：

1. `tracking_min = 0.694` ⇒ **没有任何环境因为跟踪差（<0.35）被降级** ⇒ 降级都来自"进度不够"那一条；
2. `frozen_frac = 0.353` ⇒ **1/3 的环境**处于"进度够、跟踪落在 0.35–0.80 之间"⇒ **等级保持不变**；
3. `tracking_pass_frac ≈ move_up_frac` ⇒ 晋级实际上由"`track > 阈值`"单条件决定（进度早就够：`progress_mean` 11.7 m ≫ 4 m）。

而这三类"只能慢慢过"的地形（`pyramid_stairs` 4 列 + `pyramid_stairs_inv` 2 列 + `boxes` 1 列 = **7/20 列 = 35% 环境**）
与 `frozen_frac 0.353` **恰好对得上**：它们在早期（策略还差时）被降到 ~0，之后跟踪一直在
0.65–0.80 之间徘徊 ⇒ **既不晋级也不降级，等级永久冻结**。对照上一轮的旧判据：同一列
`level_pyramid_stairs_inv` 曾到 **6.06**（A_fresh）⇒ 不是地形本身过不去。

#### 29.24.3 改动：阈值**按地形分档** + 逐列跟踪日志

* `terrain_levels_vel_logged` 新增 `relaxed_terrain_names`（默认
  `("pyramid_stairs", "pyramid_stairs_inv", "boxes")`）与 `tracking_move_up_relaxed`（默认 **0.65**）：
  这三类用 0.65，其余列仍用 **0.80**（保住"跟踪是硬门槛"的原意，同时把冻死的三类放出来）。
  `flat`/斜坡/粗糙/沟 保持 0.80 —— 它们本来就在涨。
* **逐列跟踪日志**：新增 `tracking_<地形名>`（如 `tracking_pyramid_stairs_inv`）与
  `tracking_up_threshold_mean`，这样"某一列卡住"到底是**跟踪不达标**还是**真的过不去**可以一眼区分。
* `CMoE_env_cfg.py` 里把两个新参数显式写进 `CurrTerm.params`。

#### 29.24.4 验证

* `tests/test_terrain_curriculum.py` 扩到 **6 项**：新增"同为进度 5 m、跟踪 0.70，在 `boxes` 上应**晋级**
  （0.70>0.65）、在 `flat` 上应**冻结**（0.70<0.80）"与"逐列跟踪均值被记录"两条；
  `tracking_up_threshold_mean` 断言为 (0.65+0.80)/2。
* **负向对照**：把 `relaxed_terrain_names` 清空 ⇒ 新用例立刻失败；还原后通过。
* 相关离线测试 **48 项全通过**（48 passed, 3 skipped），`py_compile`、`git diff --check` 通过。
* **未运行**：真实训练里三类地形是否真的开始晋级（需重启后观察
  `level_pyramid_stairs_inv` / `level_boxes` 与新增的 `tracking_*` 分项）。

### 29.25 「晋级阈值是不是 0.6？」——不是：**0.75**（只对三类），并说明这个代理为什么"又高又窄"

> ⚠️ **本节结论已被用户 2026-09-24 的最新决定取代（见 §29.26）**：台阶/反楼梯/`boxes` 三类最终取
> **0.50**（不是 0.75），其余仍 0.80。0.50 的实质是**对这三类取消跟踪门控**（见 §29.26 的说明）。

#### 29.25.1 实际值

```python
tracking_move_up          = 0.80   # 默认：flat / 斜坡 / 粗糙 / 沟 等 7 类
relaxed_terrain_names     = ("pyramid_stairs", "pyramid_stairs_inv", "boxes")   # 只有这 3 类
tracking_move_up_relaxed  = 0.75   # ← 本轮从 0.65 上调
tracking_move_down        = 0.35
```

#### 29.25.2 为什么不能取太低：这个代理**又高又窄**，阈值是刀尖

`track_avg` 是**整回合**的平均跟踪核，而每块 tile 有 **70–80% 是平地**
（反楼梯 tile：平台 2 m ＋ 台阶 1.8 m ＋ 平地 4.2 m ⇒ 障碍只占 **22%**）⇒ 障碍段再差也被稀释。
实测：`tracking_mean ≈ 0.78`、`tracking_min ≈ 0.69`（**全在 0.69 以上**）。
于是：

| 阈值 | 效果 |
|---:|---|
| 0.80 | 通过率约 **50%**（实测 `tracking_pass_frac 0.507`）——台阶类因此长期冻结 |
| **0.75** | 通过率约 70–80% ⇒ 台阶类能晋级，但门控**仍有牙齿** |
| ≤0.69 | **等于取消门控**（没有任何一回合低于 0.69）⇒ 这三类直接退回"走够 4 m 就晋级"的旧行为 |

⇒ 所以先取 **0.75**（不是 0.65、更不是 0.6）。

#### 29.25.3 后续（第一步已就位）

* 新增的逐列日志 **`tracking_<地形名>`**（如 `tracking_pyramid_stairs_inv`）会给出这三类的**真实分布**；
  拿到后按"该列的分布"再定阈值（例如它在 0.72–0.85 ⇒ 0.75 合适；若 ≤0.72 ⇒ 要么降到 0.65，
  要么走下一步）。
* **根治方向（未实施）**：把"整回合平均"换成**只在障碍段上统计**的跟踪均值（用 x 窗口或
  "前方射线落空"把回合切段），这样 0.6 这样的阈值才真正表示"**过障碍时**跟速好"。
* 因为 `--resume` 很便宜（不丢训练进度），调这个阈值**不需要重头训**。

### 29.26 用户最终决定：**台阶与 boxes 用 0.50 晋级，其余 0.80**

```python
tracking_move_up          = 0.80        # flat / 斜坡 / 粗糙 / 沟 等 7 类
relaxed_terrain_names     = ("pyramid_stairs", "pyramid_stairs_inv", "boxes")
tracking_move_up_relaxed  = 0.50        # ← 用户决定（不是 0.75）
tracking_move_down        = 0.35
```

#### 29.26.1 必须说清的一点：0.50 在这三类上**等价于取消跟踪门控**

实测 `tracking_min ≈ 0.69`（**没有任何一回合的整回合平均跟踪低于 0.69**）⇒ 阈值 0.50 永远成立
⇒ 这三类的晋级条件实际退化成**只看前进进度**：`progress > size[0]/2`（4 m）。

这不是笔误，也不是"松一点的门控"，而是**语义上的取舍**：

* **仍然成立的部分**：`move_down` 里"跟踪 < 0.35 就降级"那条**还在**（虽然同样几乎不触发），
  以及进度判据（走不到 4 m 不晋级、走不到 `0.5·‖cmd‖·T` 降级）仍在。
* **放弃的部分**：这三类不再要求"跟速好"。
* **为什么可以接受**：旧判据（只看进度）在这三类上曾把 A 跑推到 **level 6**（反楼梯 6.06／`boxes` 5.99、
  沟 6.10），说明"**能不能过障碍**"本来就该由**进度**来管；而跟踪门控更适合**容易地形**
  —— 那里"糊弄着走"才是失败模式，障碍地形上"慢一点但过去了"是正常且必要的。
* **代价**：就这三类而言，课程退回旧行为 ⇒ 会长到饱和（§29.12 的 `level_mean≈6` 稳态），
  但**这一次它们能真的涨上去**（这才是当前目标）。

#### 29.26.2 仍保留的观察手段与后续

* 新增的逐列 `tracking_<地形名>` 日志会把这三类的**真实分布**打出来（重启后可见）。届时可以据此判断：
  是"确实过得很糙"（分布整体偏低）还是"过得不错只是被 0.80 卡住"（分布 0.7–0.85）。
* **根治方向（未实施）**：把"整回合平均"改成"**只在障碍段上**统计的跟踪均值"（x 窗口或"前方射线落空"
  切段）。只有那样，这一类阈值（0.5/0.6）才真正表示"过障碍时跟速好"。
* 因为 `--resume` 便宜（不丢训练进度），这个阈值可以随时再调，不必重头训。

#### 29.26.3 复查（761 轮）：真正卡住的只有 `stairs_inv` 与 `boxes`，**`gap` 已脱困**

| iter | `stairs_inv` | `boxes` | **`gap`** | `level_mean` | `tracking_mean` | `tracking_min` |
|---:|---:|---:|---:|---:|---:|---:|
| 100 | 0.01 | 0.11 | 0.00 | 0.24 | 0.540 | 0.338 |
| 300 | 0.09 | 0.00 | 0.74 | 0.99 | 0.755 | 0.659 |
| 447 | 0.12 | 0.10 | **2.63** | 2.55 | 0.778 | 0.694 |
| 670 | 0.22 | 0.26 | **5.08** | 4.41 | 0.782 | 0.692 |
| **761** | **0.22** | **0.28** | **5.81** | 4.93 | 0.763 | 0.668 |

⇒ `gap` 从 0.74(300) 一路涨到 **5.81**（已接近 `flat` 5.87／`slope` 6.06）；**卡点只有 `pyramid_stairs_inv`(0.22)
与 `boxes`(0.28)** —— 正好是 §29.26 放宽名单里的两列（名单里第三个 `pyramid_stairs` 已 5.77，放宽它无副作用）。
**不需要把 `gap` 放进放宽名单**。

**这两列的跟踪区间（间接可测）**：本轮 run 的进程**没有**逐列 `tracking_<地形>` 日志（那是 2026-09-24 晚
新增的、需重启才生效）⇒ 只能给区间：

* 下界：全局 `tracking_min = 0.668` ⇒ 这两列**每个回合的整回合平均跟踪 ≥ 0.668**；
* 上界：它们没晋级，而晋级要求 `> 0.80` ⇒ **≤ 0.80**；
* ⇒ **两列跟踪 ∈ [0.67, 0.80]**，即"**差一点点**"，不是"很差"（旁证：`tracking_fail_frac` 全程 **0**）。

**这意味着**：放宽到 **0.50** 后它们会**立刻解冻**（0.5 < 0.668）；但也意味着 0.50 在这两列上
**门控彻底失效**。若想让门控在它们身上仍有牙齿，可用窗口只有 **[0.668, 0.80]** 这一小段
（≈0.72–0.75）—— 这正是"整回合平均"这个代理的固有缺陷。重启后 `tracking_pyramid_stairs_inv` /
`tracking_boxes` 会告诉我们它们落在该区间的哪一端，从而决定是"0.5 够了"还是"必须做障碍段统计"。

### 29.27 逐列步态指标（用户要求）：用三个 `GaitReward` 成对方式做成"逐列步态分类器"

用户：「我本来的期望就是，除了 box 和 gap，其他步态倾向于 trot，这两个则自己演化。是不是可以分列统计
一下步态指标，在 curriculum 指标中」。

这一诉求**日志侧原来做不到**，原因两条：
1. `feet_gait` / `joint_mirror` / `feet_air_time_variance` / `feet_height_body` / `feet_air_time`
   **都被地形掩码豁免了 `boxes`/`gap`** ⇒ 它们在这两列上的分项恒为 **0**，正好看不到"自己演化成了什么"；
2. 逐列只有 `level_<地形>`，没有任何步态量。

#### 29.27.1 设计：三种成对方式＝一个分类器（不加掩码）

同一个 `GaitReward`（2 个"同步"核 × 4 个"反相"核的乘积）换**成对方式**就得到三种步态的度量：

| 项 | 成对方式 | 对应的步态 |
|---|---|---|
| `gait_metric_trot` | `(FL↔RR)`、`(FR↔RL)`（**对角**） | trot |
| `gait_metric_bound` | `(FL↔FR)`、`(RL↔RR)`（**左右**） | bound |
| `gait_metric_pace` | `(FL↔RL)`、`(FR↔RR)`（**同侧前后**） | pace |

**谁的值最高，那一列就最接近哪种步态**；三者都贴近下界 `0.893^6 ≈ 0.508` ⇒ 既不是 trot/bound/pace
（pronk／乱走；与"站着不动"同形，可用 `tracking_<地形>` 与 `progress_mean` 区分）。

**两个关键取舍（都写进代码注释了）**：

* **权重必须非零**：`disable_zero_weight_rewards()` 会把权重 0 的 term 整个移除。故取 **1e-6**
  ⇒ 最多 `1e-6 × 1 = 1e-6/s`，相对整回合 ~4/s 占比 ~2.5e-7，**对训练无影响**（`test_check_reward_overrides.py`
  里有断言：这三项权重必须 ≤1e-5）。
* **不加地形掩码**：正是为了能看见 `boxes`/`gap` 上"自己演化"的结果。

#### 29.27.2 日志（`Episode/Curriculum/terrain_levels/` 下）

| tag | 含义 |
|---|---|
| `gait_trot_<地形>` / `gait_bound_<地形>` / `gait_pace_<地形>` | **逐列**的三种步态度量（本回合平均，8 列 × 3） |
| `gait_trot_mean` / `gait_bound_mean` / `gait_pace_mean` | 全局平均（3 个） |
| `tracking_<地形>` | 逐列跟踪均值（§29.24 加的） |
| `level_<地形>` | 逐列等级（原有） |

实现：把原来的 `_episode_tracking_average` 泛化成 `_episode_term_average(env, env_ids, term_name)`
—— 任何**奖励项**的"本回合时间平均"都能从 `RewardManager._episode_sums` 反解
（`/weight/(回合步数·step_dt)`，时机见 §29.20.2），于是步态指标**复用现成的累加机制**，不需要新写缓冲。
`terrain_levels_vel_logged` 新增参数 `gait_metric_terms=(("trot","gait_metric_trot"), …)`。

#### 29.27.3 怎么验证用户的设计意图

* **13 个非 `boxes`/`gap` 列**：期望 `gait_trot_*` **高于** `gait_bound_*` / `gait_pace_*`；
* **`boxes`(训练 1 列) / `gap`(训练 6 列)**：允许任意（包括三者都低 ⇒ 跃起/pronk 式），
  只要它们的 `tracking_*` 与 `level_*` 在涨就说明"自己演化"是有效的；
* 若出现某列 `gait_bound_*` 明显高于 `gait_trot_*`，那就是"该被约束却没被约束住"的直接证据，
  下一步就是把该列加进 `feet_gait` 的生效范围（或调整其权重/陡度）。

#### 29.27.4 验证

* `tests/test_terrain_curriculum.py` 扩到 **7 项**：新增"逐列步态度量被记录且反解回原始乘积值"的用例
  （两组成对方式、三个环境跨 `boxes`/`flat`，断言 `gait_trot_boxes=0.70`、`gait_bound_boxes=0.725`、
  `gait_trot_mean=0.70`，并断言**度量项不参与晋级判据**）。
* `tests/test_check_reward_overrides.py` 更新为 **23 项**（20 + 3 度量）并新增"度量项权重必须 ≤1e-5"的断言。
* 相关离线测试 **50 项全通过**；`py_compile`、`git diff --check` 通过。
* **需重启才生效**（新增三个 RewardTerm + 新的逐列日志都写进了启动时读入的代码/配置）。

### 29.28 用户回放 1000 轮：「都还是蹦蹦跳跳的走的，不过至少走直线了」→ 奖励结构确实在**付钱买弹跳**

#### 29.28.1 观察与代理读数一致

* **走直线 ✓**：与 §29.23.3 的三个探针一致（`|yaw|` 4.8°、`distance/progress` 1.008、`|Δy|` 0.46 m），
  "横移绕开"这条确实修掉了。
* **"蹦蹦跳跳"**：`feet_gait` 去掩码后 ≈**0.87** ⇒ **不是完整的 bound/pronk**（那种情况下 6 核乘积最高只能到
  ≈0.637）；但 `joint_mirror` 残差 **0.149**（≈满反相的 60%）、前/后对时长差 **0.154 s**、周期 ≈**0.93 s**。
  ⇒ 形态是"**低占空比（长腾空）+ 对角对之间约 0.15 s 的错开**"的对角步 —— 看着像弹跳，但不是严格 bound。

#### 29.28.2 为什么奖励结构会买出弹跳（两条）

1. **`lin_vel_z_l2` 被清零**（A 配方，理由"会与过沟所需的爆发式跃起对抗"）⇒ **trot 列上没有任何项惩罚
   竖直速度**，弹跳是免费的。
2. **`feet_air_time +1.0 @0.5` 主动奖励腾空**：其展开 `4(1−d) − 2·N落地/T` 对"滞空更久"的**梯度恒为 +1**
   （阈值只改偏移与"减少落地次数"的强度）。粗算：周期 0.9 s 时把占空比从 0.5 降到 0.35，收益增加 **≈0.6/s** ——
   这是一笔很大的正向激励，方向正是"多腾空/弹跳"。

#### 29.28.3 已落地（三项）

| 改动 | 值 | 理由 |
|---|---|---|
| **`lin_vel_z_l2` 按地形豁免地恢复** | **−2.0**（＝PPO rough 原值） | 新增 `mdp.MaskedLinVelZ`：trot 列上压弹跳，**`boxes`/`gap` 豁免** ⇒ 原"会与跃起对抗"的顾虑被掩码消掉 |
| **`feet_air_time` 降权** | **1.0 → 0.3** | 保留"别踩碎步"的作用（阈值仍 0.5 s），但把"奖励腾空/弹跳"的分量压到 30% |
| **弹跳/高度度量**（1e-6，纯记录） | `diag_bounce`（`lin_vel_z_l2` 的 1e-6 权重版） | 逐列 `gait_bounce_<地形>` ＝该列 **vz 均方**（开方＝vz RMS，m/s）；另把 `base_height_l2` 也挂进逐列度量 ⇒ `gait_height_<地形>`＝该列高度误差²（开方＝RMS） |

**生效奖励 23 → 25 项**。日志（重启后，`Episode/Curriculum/terrain_levels/` 下）：

```
gait_trot_<地形> gait_bound_<地形> gait_pace_<地形>   ← 步态分类器（逐列）
gait_bounce_<地形>                                    ← 逐列 vz 均方（开方＝弹跳 RMS）
gait_height_<地形>                                    ← 逐列高度误差²（开方＝RMS）
gait_{trot,bound,pace,bounce,height}_mean             ← 全局
tracking_<地形> / level_<地形>
```

#### 29.28.4 验证

* `tests/test_masked_terrain_terms.py`（由 `test_feet_gait_mask.py` 改名，现在覆盖两个 masked 项）扩到 **8 项**：
  新增 `MaskedLinVelZ` 的"豁免列为 0、`free=()` 时全生效"用例。
* `tests/test_check_reward_overrides.py` 更新为 **25 项**，并新增"竖直速度罚已恢复（−2.0／`MaskedLinVelZ`）"、
  "`feet_air_time`＝0.3"、"`diag_bounce` ≤1e-5"三条断言。
...
### 29.31 run G 首查（436 轮）：NaN 已除、课程分档见效，但**步态不是 trot，且相位项没有在起作用**（2026-09-24 夜）

run G＝`logs/cmoe/base_move_cmoe_rough/2026-09-24_21-23-48_cmoe_G_77dim_fresh`（21:23:48 起，
4096 环境，≈4.4 s/轮，`model_500` 尚未落地）。

#### 29.31.1 修复与配方确认

* **NaN 已彻底消失**：132 个 tag **零 NaN**；逐列 `tracking_*`／`gait_*_<地形>` **353/353 轮都有点**
  （不是稀疏断点）——§29.30 的修法（有样本才写键 + runner 按键并集遍历）在真实训练里成立。
* 训练健康（前 50 轮 → 最近 50 轮）：速度核 **0.200 → 0.617**（峰值 0.656@250）、`ep_len` 705 → 885、
  `illegal_contact` 0.274 → 0.260（100 轮峰值 0.432）、`mean_noise_std` 0.94 稳、`lr` 2.3e-3 → 2e-4（没贴地板）、
  `tracking_mean` 0.144 → 0.721、`progress_mean` −0.47 → **+9.39 m/回合**、`level_mean` 1.14 → 1.93（末值 2.23）。
* **课程分档改动确实见效**（同龄窗口 300–356 对比 run C）：`pyramid_stairs_inv` **0.10 → 2.93**
  （C 到 1000 还卡在 0.30，正是用户上次的痛点）、`pyramid_stairs` 0.56 → 3.72；代价是 0.8 档的
  四类同龄偏低（`flat` 2.91 vs 3.58、`gap` 0.61 vs 1.05）且摔率更高（0.265 vs 0.172）——这是
  "砍掉 0.7/s 腾空付款 + 新增 vz 罚"的直接代价，不是 bug。

#### 29.31.2 步态判读（本轮重点）

三个逐列分类器（`gait_metric_{trot,bound,pace}`，权重 1e-6，同一 `GaitReward` 换"成对方式"）在 436 轮时：

| | trot | bound | pace |
|---|---|---|---|
| 全局均值 | 0.9266 | **0.9364** | 0.9265 |
| 逐列（8 列） | 0.915–0.950 | 0.926–0.956 | 0.915–0.950 |

**结论一：不是 trot，且分类器根本没在区分。** `trot−bound` ＝ **−0.010 ± 0.002**，而且从第 50 轮
（−0.0097）到 436 轮（−0.0098）**完全平坦**——期间速度核从 0.20 涨到 0.62（整体能力大幅提升），
唯有这个差值不动。若相位项在起作用，它付钱买的正是"trot 高、bound 低"，这个差值应当收窄；
实测不动 ⇒ **相位项对步态没有产生任何影响**。`trot−pace` 更已收敛到 +0.0001（两者重合）。

**结论二：为什么它不动——6 核乘积在当前参数下已经饱和。** 依 `mdp/rewards.py` 的
`GaitReward._sync/_async_reward_func`：`se = clip(Δ², max=max_err²=0.04)`（两项相加）、
`kernel = exp(−se/std)`、`std = √0.5 = 0.7071` ⇒ 单核区间 **[exp(−0.08/0.7071), 1] = [0.893, 1]**，
6 核乘积区间 **[0.508, 1]**。也就是说：**只要某一对的 air/contact 时间差超过 `max_err`＝0.2 s，
该核立刻贴到 0.893 的地板**，再差也还是 0.893 ⇒ 这个项度量的是"有几对配对了"（近似二值），
而不是"差多少"。而当前 `trot−bound` 只差 0.010 ⇒ 等价于 6 个核里只有约 **0.1 个核**的差别
（`0.893^k`，k 为"配错"的核数）⇒ 相位项给出的边际奖励 ≈ **0.01/s**（权重 1.0），
相对总量 ~5.5/s 是 **0.2%**，PPO 不会因此改步态。

**结论三：把精确核放进参考步态里算，才能看出"能区分多少"。** 离线按同一公式（T＝0.93 s、
占空比 0.5）算参考步态：

| kernel 参数 | 参考 trot | 参考 bound | pronk | trot−bound |
|---|---|---|---|---|
| **现在** `max_err=0.2, std=0.7071` | 1.000 | 0.736 | 0.736 | **+0.264** |
| `0.2, 0.2` | 1.000 | 0.395 | 0.395 | +0.605 |
| `0.5, 0.2`（§29.28 的方案） | 1.000 | 0.302 | 0.302 | **+0.698** |
| `0.5, 0.15` | 1.000 | 0.262 | 0.262 | +0.738 |

⇒ 步子迈对时这个项**能**给出 0.26/s 的 trot 溢价（当前参数），把脊变陡可提到 0.70/s（2.7×）；
但实测的多度量都在 0.93、彼此只差 0.01，说明**实际步态落在一个"三种配对都算合格"的区域**——
即四足的时间尺度又短又接近（`feet_air_time_variance` 反解出的四足 `last_air_time` 散布只有
**RMS 0.078 s**，同为"步态项里权重最大"的 −8.0 项在奖励"四足时长相等的近似 lockstep"），
短而整齐的步子会让**所有**核对都 ≈1，从而同时喂饱 trot/bound/pace 三个度量。
这也解释了 `feet_air_time` 那个"看起来矛盾"的数字：它只约束乘积 `N_落地·(ā − 0.5)`（实测 ≈ −190），
`N_落地` 与 `ā` 不可分辨 ⇒ "长步幅慢步（ā≈0.45 s，4 次落地/s）"与"短步快蹭（ā≈0.05 s，23 次落地/s）"
都能对上，必须靠新诊断项才能分开。

**判读：现在的相位项不是"trot 塑形"，而更像"短而整齐的步子"的塑形**（后者拿满分，且不需要 trot 的
对角关系）⇒ 继续训练不会长出 trot，因为它已经拿到了 0.93/1.0 的分。物理上这对应"低抬脚、高频、
四足近乎同步"的蹭/蹦步态，与用户回放 run C 时看到的"蹦蹦跳跳"同族。

#### 29.31.3 建议（待用户定）

1. **把相位核变陡**：`feet_gait`（以及三个 `gait_metric_*`，保持分类器可比）`max_err` 0.2→**0.5**、
   `std` 0.7071→**0.2** ⇒ trot−bound 溢价 0.26 → **0.70/s**（2.7×），且非 trot 步态不再拿 0.74。
2. **给"短步"反压**：`feet_air_time` 现为 0.3（§29.28 为治蹦而砍）。它（阈值 0.5）是唯一要求
   **长滞空／少落地**的项；但它是"奖励腾空"的那一项，与新的 `lin_vel_z_l2 −2.0` 方向相反。
   **先加诊断再决定**，不盲调。
3. **新增接触时序诊断项**（权重 1e-6，纯记录，与 `diag_bounce` 同法）：①`ā`＝四足 `last_air_time` 均值
   （定"长步幅 vs 短步快蹭"）；②六对 `|Δair| + |Δcon|` 的均值（定相位差的真实尺度，用来按实测选
