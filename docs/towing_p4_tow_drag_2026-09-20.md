...
用户提出「照理说我们的绳子应该不是很弹性」。用已有数据量化（`k=4000 N/m, c=100 N·s/m`）：

| 量 | 实测 | 说明 |
|---|---|---|
| 稳态伸长 `T/k` | **0.70~2.70 mm**（L0=0.8 m 的 0.09%~0.34%） | 稳态几乎不可伸长 |
| 绷直瞬间峰值张力 | **60~84 N** | 稳态只有 10.2 N（b=0.032） |
| 过冲比 峰值/稳态 | **5.9×~24.8×** | 明显欠阻尼弹簧行为 |
| 峰值伸长 | 15~21 mm | 相当于绳突然「让出」2 cm |
| 纹波 | 6~8 Hz | 步态频率经 `c·ḋ` 传入，不是绳的固有频率 |

理论对照：折合质量 `μ = ((1/m_eff)+(1/m_robot))⁻¹ = 3.63 kg`，
`ω = √(k/μ) = 33 rad/s`（5.3 Hz），`ζ = c/(2√(kμ)) = 0.083~0.24`（欠阻尼），
绷直能量界 `½μv² = ½kδ²` ⇒ δ=26 mm、峰值 ≈104 N，与实测 60~84 N 同量级。

真实绳子的轴向刚度 `k = EA/L`：6 mm 涤纶/尼龙绳 EA≈50~100 kN ⇒ k≈6×10⁴~1.2×10⁵ N/m；
4 mm Dyneema ⇒ k≈1.6×10⁶ N/m。**当前 4000 N/m 比它软 15~800 倍**，更像弹性拖车带。

显式弹簧的稳定条件 `dt < 2/(ω(ζ+√(ζ²+1)))`：

| k (N/m) | ω (rad/s) | dt 上限 | 5 ms 是否可用 |
|---|---|---|---|
| 4×10³（当前） | 33 | 47 ms | 可用（余量 10×） |
| 1×10⁵（≈6 mm 绳下限） | 166 | 11 ms | 勉强 |
| 1.6×10⁶（Dyneema） | 525 | 3.8 ms | **失稳** |

所以**在「5 ms 步长 + 显式弹簧」这个框架里表示不了真实绳的刚度**；而 k 提到 1×10⁵ 时
绷直峰值会从 ~70 N 涨到 ~300 N（能量界 `v√(kμ)`），机器人被猛拽（对照：§5.9 里 236 N
就把机器人拽翻）。**注意这不会推翻已有结论**：稳态张力 `4bω/r`（实测 10.60 / 2.85 N 吻合）
与滑行段都与 k 无关，只有绷直瞬态受影响。

**待验证**（用户跑）：三档 `k`（4000 / 40000 / 100000）× 两种 `c`（固定 100 / 临界阻尼
`2√(kμ)`）在同一 case 下对照，看绷直峰值、过冲比与稳态张力是否如预期不变。
默认值**暂不改**——等实测数据回来再定。
