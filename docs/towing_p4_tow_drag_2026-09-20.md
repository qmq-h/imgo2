...
**待验证**（用户跑）：三档 `k`（4000 / 40000 / 100000）× 两种 `c`（固定 100 / 临界阻尼
`2√(kμ)`≈1528 N·s/m）在同一 case 下对照，看绷直峰值、过冲比与稳态张力是否如预期
（峰值应随 √k 涨、稳态张力不变）。默认值**暂不改**——等实测数据回来再定。
...
- **P4 的核心行为已验证**（§5.5：`v_R ≈ v_L ≈ 0.512`、T ≈ 5.2 N 稳定、小车被拖 2.47 m），
  ...
  - **绳刚度 `k` 的敏感性**（§5.18）：现用 `k=4000 N/m`，比 6 mm 涤纶绳软 15~800 倍；
    三档 k 的对照**未跑**。`k` 提到真实量级会与 5 ms 步长的显式积分冲突（dt 上限 3.8~11 ms）。
