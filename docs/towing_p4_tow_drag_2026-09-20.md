...
| `min_gap_after_tow_start_m` | **拖曳段开始后**的最小挂点间距（station 段的近距是设计值，不计入） | ≤ 0 ⇒ `load_reached_robot`（追尾） |
| `final_gap_m` | **最终停在离机器人多远**（末帧挂点间距，即两车相对面的净空） | 只报告 |
| `gap_closed_m` | 初始间距 − 最终间距 | 只报告 |
| `caught_up` | 是否追到机器人（布尔） | 只报告，与 `load_reached_robot` 同源 |
| `predicted_min_gap_m` / `prediction_error_m` | 启动预判 vs 实测最小间距 | 只报告（预判只建模滑行段） |
| `coast_load_travel_m`（对照预测） | 滑行段小车实际滑行距离 | 应与启动时打印的预测值接近（预测式在 P2 吻合 1% 以内） |
...
| `caught_up` | 是否追到机器人；为真时判 `load_reached_robot` 失败 |
