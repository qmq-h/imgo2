...
### 4.1 追尾基线：未来训练目标要对标的量

**设计前提（用户 2026-09-21 明确）**：当前框架测试阶段**追尾本身就是需要的结果**；
后续训练的目标是「**收到停止指令后机器人自动往前几步，防止追尾**」。所以下面这三个量
就是那个策略要对标/超出的基线——尤其**时间窗**，它决定「往前几步」必须在多久之内完成。

新增报告量（`summarize_tow.py`，只需关节列 + coast 段即可离线复算）：

| 字段 | 含义 |
|---|---|
| `clearance_at_stop_m` | **指令归零那一刻**的车头间隙（机器人可用的初始余量） |
| `time_to_contact_after_stop_s` | 从那一刻到**首次接触**（间隙 ≤ 0）的时间窗；未接触为 `None` |
| `load_vx_at_contact_mps` | 接触瞬间负载的速度（**撞击速度的实测值**） |

已有 run 复算出的基线（`v_cmd = 0.5`、L0=0.8、`--slack 0.40`）：

| 配置 | 归零时间隙 | **到接触时间窗** | **接触速度** | 末车头间隙 |
|---|---|---|---|---|
| m5 b=0.032 | 0.480 m | 未接触 | — | +0.346 m |
| m10 b=0.032 | 0.479 m | 未接触 | — | +0.200 m |
| m15 b=0.032 | 0.486 m | 未接触 | — | +0.090 m |
| m20 b=0.032 | 0.486 m | **2.675 s** | 0.041 m/s | −0.025 m |
| m25 b=0.032 | 0.485 m | **1.775 s** | 0.131 m/s | −0.038 m |
| m10 b=0.008（f=0.4/0.8/1.2） | 0.473–0.480 m | **1.050 / 1.100 / 1.175 s** | 0.358 / 0.324 / 0.315 m/s | −0.014…−0.026 m |

**`v_cmd = 1.0 m/s` 的实测（run `20260921T112517Z_133d2323`，b=0.008、5 个质量全部追尾）**：

| 质量 | 跟速比 | 稳态 T | 归零时间隙 | **时间窗（实测 / 预测）** | **接触速度（实测 / 预测）** | 最小车头间隙 | 车斗受力峰值 |
|---|---|---|---|---|---|---|---|
| 5 kg | 1.011 | 5.59 N | 0.4658 m | **0.725 s** / 0.65 s | **0.531** / 0.54 m/s | −0.0338 m | 309 N |
| 10 kg | 1.008 | 5.56 N | 0.4649 m | **0.625 s** / 0.58 s | **0.775** / 0.73 m/s | −0.0354 m | 635 N |
| 15 kg | 1.009 | 5.60 N | 0.4679 m | **0.600 s** / 0.54 s | **0.859** / 0.79 m/s | −0.0421 m | 819 N |
| 20 kg | 1.009 | 5.59 N | 0.4580 m | **0.575 s** / 0.52 s | **0.904** / 0.83 m/s | −0.0874 m | 258 N |
| 25 kg | 1.008 | 5.60 N | 0.4593 m | **0.575 s** / 0.51 s | **0.926** / 0.85 m/s | −0.0527 m | 1007 N |

**五条结论**：

1. **1.0 m/s 下 b=0.008 的 5 个质量全部追尾**（`min_clearance` −0.034~−0.087 m，三路见证全中），
   与 §4 的预判表一致；
2. **时间窗 0.575~0.725 s**（预测 0.51~0.65），**接触速度 0.53~0.93 m/s 与解析预测差 2~9%**
   ——比 0.5 m/s 时的 ~25% 好得多（速度越高，接触口径的 0.46 m 假设误差占比越小）。
   ⇒ 这套解析式可以当**训练前的设计工具**：给定 L0、b、质量与指令速度，先估出「往前几步要抢多久」；
3. **跟速比 1.008~1.011（101%）**，而 0.5 m/s 时只有 0.94 ⇒ **底层策略在高速段反而更准**，
   与 AMP 调参记录里「低速度段是最弱区」一致。1.0 m/s 这一档因此比 0.5 m/s 更好用；
4. **稳态张力 5.56~5.60 N** ≈ `4bω/r` = 5.0 N（与 0.5 m/s 时 2.85 对 2.5 的比例一致，系统性高约 12%）；
5. **注意 `final_clearance_m` 在追尾 run 里没有"停车距离"的含义**：撞击把机器人往前推，
   所以末间隙反而变正（+0.05~+0.38 m）。追尾 run 要看 **`min_clearance_m`（压进去多深）**、
   `time_to_contact_after_stop_s`、`load_vx_at_contact_mps` 与 `deck_contact_peak_n`。

**三条可用的结论**：

1. **归零时的车头间隙几乎是常数 0.48 m**（0.473–0.486，跨质量与轮阻）——因为那一刻绳正好在
   L0=0.80 绷着、机器人姿态相近。所以「往前几步」要补的就是**约 0.48 m 的间隙**（或等价地：
   把负载的相对动能耗掉）。这也是 L0/`--slack` 的直接后果，改这两个参数就会改这个常数；
2. **时间窗在 1.0–1.2 s（b=0.008 这种"快追尾"）到 1.8–2.7 s（b=0.032/重载）之间**；
   按同一套解析式推 **1.0 m/s 时缩到约 0.54–0.58 s**（b=0.008）——速度翻倍、时间窗近似减半，
   所以训练时的策略必须在**亚秒级**内完成「往前几步」；
3. **接触速度**是撞击强度的实测值：b=0.008 是 0.32–0.36 m/s（0.5 m/s 指令），b=0.032 只有
   0.04–0.13 m/s。解析预判（`v_hit = v0·e^(−t/τ)`，接触口径取 0.46 m）给 0.26 m/s 对 m10/b=0.008，
   与实测 0.32–0.36 差 ~25%——**时间窗比撞击速度更可信**，策略设计应以时间窗/间隙余量为主。
...
**可视化命令的三个档（都实测过，「不加 `--headless` 就会渲染」）**：

| 用途 | 命令要点 | 会看到什么 |
|---|---|---|
| **只验管道**（约 20 s 墙钟） | `--duration 1 --stop-at 0.5 --settle-time 0.2` | UI 起来、4 个 env 建好、逐 env 落盘。**绳全程松弛**（拖曳段 0.5 s < 绷直所需的 0.8 s）⇒ 张力 0、判 `rope_never_taut_during_tow`，**测不出物理**，别误判成 bug |
| **能看到物理**（约 3 s 仿真） | `--duration 3 --stop-at 2` | 站定 → 走 0.4 m **绷直** → 拖行 → 机器人停、小车滑行；两套模型的绷直差别可见 |
| 完整 | `--duration 10 --stop-at 5` | 与验收同长度 |

> **为什么至少 3 s**：`--velocity 0.5` 下机器人要走 `--slack 0.40` m ⇒ **0.8 s** 才把绳拉直；
> 拖曳段短于这个数就什么都看不到（实测 `--duration 1 --stop-at 0.5` 就是这种情况）。

**已知限制（用户 2026-09-21 决定不重构）**：多环境这条路的**形状/设备约定没有离线覆盖**
（`main()` 里的批量施力与逐 env 记录只能在有 PhysX 的进程里执行），所以改动它必须实跑验证——
这一点已经用第 4~8 次失败换来了教训。**不重构的理由**：该入口是**临时测量/可视化台**，
训练侧（P6/P8）会另建环境；而给训练复用的**库**代码（`mdp/rope_model.py`、`SplitRopeModel`、
批量 API）都是离线验证过的（见 §5.20）。
