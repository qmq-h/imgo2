...
| AMP-09 | P1 | **待用户决定**（一项）：actor 观测是 45 维（保留 `base_ang_vel`）还是收到 42 维与参考逐字一致 | 2026-09-18 用户要求「除关节 kp/kd 以外都对齐」后，rlamp 两个任务已在奖励配方、观测缩放、动作缩放/裁剪、指令范围、域随机化、reset 分布、观测噪声、`min_normalized_std` 上逐项对齐（[§15.1/§15.2](docs/amp_gait_adjust_plan_2026-09-18.md)）。**仍不一致且必须由用户拍板的是 actor 观测集**：参考 `rl_amp` 的 actor 是 `privileged_obs_buf[:, 6:]`（`legged_robot_amp.py:269`），**既不看线速度也不看角速度** ⇒ 42 维；我们是 45 维（AMP-05 只去掉线速度、保留 IMU 角速度，用户此前的决定）。**两个选项的代价**：① 保持 45 —— 与参考差一个 3 维块，但部署契约（`amp/config.yaml` 的 `observations` 列表 + C++ 观测拼接 + 已部署 24500 策略）不变；② 改成 42 —— 与参考逐字一致，但要同步改 `imgo2_deploy/policy/imgo2/amp/config.yaml`、C++ 观测拼接与导出复核，且**必须重训**（旧 checkpoint 第一层是 45 维，不得截网络）。另：参考的 `noise.noise_scales.ang_vel = 0.3` 之所以在参考里无效果，正是因为 actor 不含角速度；若改成 42，那条噪声对齐也随之变成死代码 | 拍板后按单变量执行（改 actor 观测 → 新任务/新 run → 重训 → 重导出 → 三条契约复核）；在此之前本轮 rlamp 训练按 45 维进行 |
