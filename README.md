...
> 最后核对：2026-09-21（P1/P2 验收通过；P3 绳力、P4 契约/适配器/场景/拖曳入口已实现，**P4 核心行为已在训练机验证**——0.5 m/s 下 `v_R ≈ v_L ≈ 0.512`、张力 5.2 N 且漂移 1.1%、小车被拖 2.47 m；本轮更正「小车有初速度」的根因并修掉：机器人出生窜动把绳拉直（峰值 85.7 N），改为出生高度 0.30 m + 松弛 0.20 m + **拖曳段前显式重摆小车清零速度**；新增 `--stop-at`（P6 阶跃停止，含追尾风险指标）。全量离线测试 **211 项**、三个校验脚本退出码 0。剩余：1.0 m/s、`m_L` 包线扫描、PPO 契约）。本文是整个工作区的维护入口，覆盖项目总览、训练部署流程、已知问题和变更记录。
...
> （[Gazebo 记录](docs/gazebo_ros2_bringup_2026-09-17.md) 第 9 节）、拉取 `b9a4913` 后对
> 部署接口 ↔ 训练配置的离线核对（同记录 9.5 节），以及按用户要求把 rlamp 两个任务
> **逐项对齐到参考 `rl_amp`**（[AMP 步态调整方案](docs/amp_gait_adjust_plan_2026-09-18.md)
...
| CART-01 | P1 | **已完成（P1/P2 全部验收通过，训练机实测）** | 离线 5 项检查两台机器一致通过（退出码 0）。**2026-09-20 训练机（RTX 4090 / Isaac Sim 5.0 / IsaacLab 0.45.9）验收**：① **P1 落地通过**——`state=completed` `valid=true`、四轮 `contact_fraction` 0.9734（未接触的 2.66% 全是 3 cm 掉落瞬态，落地后全程接触）、静止高度 **0.150000 m**、tilt 9.8e-6、导入质量 9.99999964 kg、刚度/阻尼恰好 0、模型哈希 `866934f6…`；② **P2 单工况 `v=1.0 b=0.016` 通过**——1.0→0.00027 m/s、`stop_time` 4.215 s、**停止距离 1.053 m**、四轮接触 1.0、滚动残差 1.2e-4、正功 0.0 J，与解析黏性衰减吻合 **0.3%**，逐点 `tau≈-b·ω_上一帧` ⇒ **力矩确在 PhysX 生效**；③ **dt 复核通过**——`--dt 0.0025` 对 dt=0.005 停止距离差 **0.23%**、停止时刻差 0.12%、接触均 1.0；④ **8 case 阻力扫描通过**——停止距离随 b 单调下降（0.5 m/s：1.034→0.516→0.257 m；1.0 m/s：2.111→1.053→0.524 m），`distance_decreases_with_damping=true`（两组），`b=0` 阴性对照 10 s 仅掉 3e-5 m/s 且 `stopped=false`，`calibration.json` 候选 `[0.016, 0.032]`（1.0 m/s 落在 0.5–2 m）；六个已停止工况与解析解吻合 **0.27%–0.96%**。⑤ 顺带在真实截断产物上验证「残缺运行不得重标为有效」（`valid=false`、退出码 1）。详见 [检查与验收记录](docs/cart_p1_p2_checks_2026-09-20.md) | 验收已按实现记录 §6 完成。后续：P3 及以后（绳索张力、上层策略）；操作前提是 **Isaac Sim 串行执行**（并发实例争锁会中断，见记录 §3.5）；遗留 CART-02/03 |
| CART-02 | P1 | 已定位，待修复（本机可复现） | **初始化失败时入口退出码为 0**：2026-09-20 三次尝试（drop/coast/drop）真实退出码全为 0 且无 traceback，唯一失败信号是 `experiment.json` 的 `state:"failed"` ⇒ 只看 `$?` 会把「Isaac Sim 没起来」当成功。根因：`cart_coast.py:224-230` 记录 `failed` 后 `raise`，但 `finally: application.close()` 先执行，而 `SimulationApp.close()`（`simulation_app.py:626`）内部 `app.shutdown()` 由 Kit/Carb 直接终止进程，Python 层收不到控制权。详见 [检查记录 §3](docs/cart_p1_p2_checks_2026-09-20.md) | 失败路径显式带非 0 码退出（如 `os._exit(2)`），并断言 `experiment.json` 仍记 `failed`；无 GPU 环境即可验证 |
| CART-03 | P1 | 已定位，待确认是否需要修 | `summarize_cart_coast.py` 对「只含 `experiment.json` 的失败目录」抛未捕获 `FileNotFoundError`（缺 `config.json`），真实退出码 1、无友好提示；`--help` 正常，「残缺运行不得重标为有效」的行为由单测覆盖不受影响。详见 [检查记录 §3](docs/cart_p1_p2_checks_2026-09-20.md) | 需要时在入口前置检查 `config.json`/`trajectory.csv`，缺失则打印可读错误并非 0 退出；或明确记录「该输入不在支持范围」 |
| TOW-01 | P1 | **P3 已实现；离线验证通过，并已由 P4 拖曳运行在训练机完成物理验证** | 计划 P3「实现绳索」：新增 `towing/mdp/rope.py`，实现单侧弹簧阻尼 `T = 0（δ≤0）/ max(0, kδ+cḋ)（δ>0）`、力对 `F_R=+T·e` 与 `F_L=-T·e`、以及力臂力矩（挂点帧会被合并进 `base_link`，所以力施加到车体并补 `offset×F`）。纯算术、不依赖 torch / Isaac Lab，**31 项标准库测试通过**（动量守恒到 1e-9、总机械能不增且松弛段动能漂移恰为 0、`δ≤0 ⇒ T=0` 零违反、numpy/torch 后端一致、`mdp` 包可脱离仿真器导入），全量离线测试 **142 项通过**（此前 111 项）。**两条模型性质**：① `δ≤0 ⇒ T=0` 成立但**反向不成立**——δ>0 而快速接近时 `kδ+cḋ` 变负被 `max(0,·)` 截断（20000 步中 250 步）⇒ 后续判「绳是否在拉」必须用 `T>0`，不能用 `d>L0`；② 自由双质点模型下松弛段冻结相对速度，**提高阻尼也不能收敛到同速**（ζ=1.01 时 4 s 后仍剩 −0.085 m/s，97.6% 步数处于松弛），这是模型性质、**不可外推到 P4**（那里机器人由冻结策略驱动，是带限权限的速度源）。详见 [P3 记录](docs/towing_p3_rope_2026-09-20.md) | 已完成。剩余：`k`/`c`/`L0` 的标定与敏感性扫描（P4 的拖曳用的是 P3 记录里的工程起点） |
| TOW-02 | P1 | **P4 核心行为已验证：0.5 m/s 稳定拖曳成立**；1.0 m/s 与 m_L 包线扫描待做 | 按计划 P4「先完全不改 locomotion」实现并**在训练机验证**（[P4 上半](docs/towing_p4_policy_contract_2026-09-20.md)、[P4 下半](docs/towing_p4_tow_drag_2026-09-20.md)）：契约+适配器、场景、拖曳入口、记录器；判读逻辑在标准库工具 `scripts/tools/summarize_tow.py`（可用真实轨迹离线复算）。**第四次实跑结果**（`--velocity 0.5`，L0=1.0/k=4000/c=100，轮阻 0.016）：`steady_robot_vx` **0.5123**（跟速 **102%**）、`steady_load_vx` **0.5123**、`speed_gap` **−6.3e-5**（计划的头号判据 `v_R ≈ v_L` 成立）、`steady_tension` **5.199 N**、`tension_drift_ratio` **1.1%**、收松弛后松弛占比 **0**、绳长 1.0013 m（伸长 1.3 mm ⇒ `kδ=5.2 N` 与实测吻合）、小车被拖 **2.47 m**、pitch 3.2°/11.1°、站高 0.284 m（**0.35 m 出生可用，未复现「高/低站高分支」**）、判定 `valid=true`。**沿途修掉的 4 个真实缺陷**（全部由实跑暴露，细节见记录 §5）：① Isaac Lab 的 DOF 顺序（广度优先）≠ 策略顺序（逐腿），必须显式置换；② 运行目录被入口与 recorder 重复创建（`FileExistsError`）；③ 挂点**位置**漏加偏移 + `--slack` 写成了预张紧 ⇒ 初始张力 **1955 N** 把机器人拽倒；④ 判据有洞（两体都静止时 `speed_gap` 也通过）＋「张力变异系数」把步态纹波误判为失稳，已改为**漂移判据**。**已定位并修正（此前诊断有误）**：站定阶段小车漂 0.239 m 并非接触自漂（加 `--cart-drop 0.03` 后仍是 0.2375 m），真因是机器人按 0.35 m 出生（实测站高 0.284 m）落地时**向前窜动 0.849 m/s**，把绳从 0.95 m 拉过 `L0` ⇒ 站定阶段绳被拉直（**峰值 85.7 N**）把小车甩到 0.369 m/s 后自由滑行。修法三层：出生高度默认 0.35→**0.30 m**、`--slack` 默认 0.05→**0.20 m**、**拖曳段开始前显式重摆小车并清零速度**（初始条件由设计决定）。判据新增 `rope_taut_during_settle`，用真实轨迹复算两条既有运行都被正确判失败。另新增 `--stop-at T`（阶跃停止，计划 P6）：`phase` 列分 `station`/`tow`/`coast` 三段，滑行段报告小车滑行距离、**最小间距（追尾风险）**、张力归零耗时与是否重新绷紧。 | 1.0 m/s 与 `m_L = 5/10/15/20/25 kg` 包线扫描（缩放质量须同时缩放惯量）；确认 `--cart-drop` 是否消除站定自漂；之后才是 P5（步态类 Logger）、P6（阶跃停止）与上层策略。 |
...
| 日期 | 变更 | 验证与限制 |
|---|---|---|
| 2026-09-21 | 按用户要求把初始间距拉近：绳在机器人走一小段后才发力 | `--slack` 默认 0.20 → **0.40 m**。`间距 = L0 − slack` 所以 slack 既表示「松了多少」，也等于「机器人要走多远绳才发力」，且 slack 越大两者初始越近：小车初始 x −1.196 → **−0.991 m**、挂点间距 0.80 → **0.60 m**、绳发力前机器人走 **0.40 m**（0.5 m/s 下 0.8 s）。**新增可核对读数** `takeup_time_s` / `takeup_robot_travel_m`（设计上应 ≈ `--slack`），并把合成轨迹测试改为**按速度积分位置**（此前 `robot_x` 是常数，位移类指标测不出来）。**副作用记录**：张紧瞬间相对速度 ≈ 指令速度，阻尼项 `c·ḋ` 会立刻给出 ≈50 N 冲击（稳态才 ~5 N）——这是 `c = 100 N·s/m` 的直接后果，不是 bug，想柔和就调小 `--damping`；`k`/`c` 仍未标定（与 TOW-01 的待办一致）。验证：全量离线测试 211 → **213 项通过**。详见 [P4 下半记录 §5.7](docs/towing_p4_tow_drag_2026-09-20.md) |
| 2026-09-21 | 更正「小车有初速度」的根因并修掉：机器人出生窜动把绳拉直；新增阶跃停止（P6）| 用户看可视化后指出「小车有初速度是不对的」。逐帧核对运行 `3746654e`（已含 `--cart-drop 0.03`）：**`--cart-drop` 无效**（自漂 0.2390→0.2375 m），**真因是机器人按 0.35 m 出生**（实测站高 0.284 m）先落 6.6 cm、**落地前窜 0.849 m/s**，把绳从 0.95 m 拉过 `L0=1.0` ⇒ **站定阶段张力峰值 85.7 N、17 个样本 T>0**，把小车甩到 0.369 m/s 后自由滑行 0.238 m。此前我只抽查几个时间点就断言「站定阶段绳是松的」，结论错误，已更正。**修法**：① `ROBOT_SPAWN_HEIGHT_M` 0.35→**0.30**；② `--slack` 0.05→**0.20**（实测窜动多拉长约 0.063 m）；③ **`reset_cart_for_tow()`**：拖曳段开始前按机器人**当时**位置把小车摆到「挂点间距 = L0 − slack」并清零线/角速度与轮速，初始条件由设计决定（与 P1/P2 静置后重写状态的约定一致）。**判据**：新增 `rope_taut_during_settle`（station 段张力 >1 N 即失败）+ `settle_max_tension_n`/`settle_max_abs_robot_vx_mps` 报告量——用真实轨迹复算，两条既有运行都被正确判失败（85.7/75.1 N）。**新增 `--stop-at T`**（计划 P6 阶跃停止）：`phase` 列分 station/tow/coast，滑行段报告 `coast_load_travel_m`、`coast_min_gap_m`（追尾风险）、`coast_time_to_slack_s`、`coast_retension_peak_n`；判据加 `robot_did_not_stop`/`load_reached_robot`。文档补了可视化命令（去掉 `--headless` 即可，`step()` 默认渲染）与相机取景提醒。验证：全量离线测试 199 → **211 项通过**；三个校验脚本退出码 0。详见 [P4 下半记录 §5.6](docs/towing_p4_tow_drag_2026-09-20.md) |
| 2026-09-20 | **P4 拖曳成立**：0.5 m/s 下 `v_R ≈ v_L`、张力稳定，小车被拖 2.47 m | 运行 `20260920T144620Z_47cbd7ff`：`steady_robot_vx=0.5123`（指令 0.5，跟速 102%）、`steady_load_vx=0.5123`、`speed_gap=−6.3e-5`、`steady_tension=5.199±1.499 N`、`tension_drift_ratio=1.1%`、收松弛后松弛占比 0、`steady_distance=1.0013 m`（伸长 1.3 mm ⇒ `kδ=5.2 N`，与实测张力吻合）、小车位移 −1.339→+1.136（**2.47 m**）、pitch 均值 3.2°/最大 11.1°、站高 0.284 m ⇒ **`spawn-height 0.35` 可用**（未复现「高/低站高分支」），判定 `valid=true`。**本轮改掉两处**：① 判据把「张力变异系数 ≤25%」当稳定判据，实测 28.8% 误判失败——但纹波主频 7.5 Hz、弹簧项只波动 1.06 N，波动来自阻尼项 `c·ḋ`，属步态正常纹波；计划要的是「相对稳定**区间**」，故改为**前后半均值漂移 ≤25%**，纹波只报告（真实轨迹复算漂移 1.1%）；判读抽到标准库工具 `scripts/tools/summarize_tow.py`，可用真实轨迹离线复算。② **新暴露**：站定阶段（指令 0、绳松弛）小车自漂 0.239 m/峰值 0.372 m/s，推断是「按 resting height 精确贴地生成」的接触自漂（P1/P2 留 3 cm 落差），已加 `--cart-drop`（默认 0.03）并把 `settle_load_drift_m` 记入 `summary.json`，**待下次确认**。验证：全量离线测试 188 → 199 项通过；三个校验脚本退出码 0。详见 [P4 下半记录 §5.5](docs/towing_p4_tow_drag_2026-09-20.md) |
| 2026-09-20 | P4 **第一次完整跑通**（流程全通、行为失败）：修掉初始条件的两处 bug 并加强验收判据 | 第三次实跑 `a03574f3` 完整跑完（`state=completed`），管线全通。**行为失败**：`steady_robot_vx=0.0005`（指令 0.5）、`tension_peak=0.0`、`tension_slack_fraction=1.0`、`max_abs_pitch=0.802`。轨迹显示**初始挂点间距实测 1.4777 m**（设计 1.05）、**初始张力 1955 N**（机器人自重 54 N）⇒ 0.1 s 内 pitch −0.74 rad、t≈0.5 s 被拽倒 ⇒ 之后 v_R≈0。**两处 bug 叠加**：① 算挂点**速度**时加了偏移、算**位置**却用了刚体原点 ⇒ 绳长混进 0.35 vs 0.15 m 的高差；② `--slack` 名字是松弛、实现是「L0 + slack」＝预张紧（5 cm 对应 200 N 预载）。**修法**：挂点世界坐标 = 刚体原点 + 旋转后偏移；`initial_cart_x()` 按三维几何解算（横向 √(target²−dz²)，并检查几何不可能的情形）；离线核算 slack=0.05 ⇒ 间距 0.9500 m、**初始张力 0 N**、前进 0.05 m 才张紧。**判据加强**（原判据有洞：两体都静止时 `speed_gap` 只有 0.028 也通过）：加 `steady_tracking_ratio ≥ 0.5` 与 `tension_slack_fraction ≤ 0.9`；日志加 `robot_z_m`/`load_z_m`（「倒了」此前只能从 pitch 间接看出）。新增回归 `test_initial_condition_leaves_the_rope_slack`（断言 **t=0 张力为 0**）等 ⇒ 全量离线测试 186 → 188 项通过。**未验证**：机器人能否在 `spawn_height=0.35` 稳定站立并跟住 0.5 m/s —— 下次的**站定阶段**（t<1 s、绳松弛、张力 0）会直接给出答案。详见 [P4 下半记录 §5.4](docs/towing_p4_tow_drag_2026-09-20.md) |
| 2026-09-20 | 按计划 **P4 下半**：机器人+小车+绳的拖曳场景与实验入口 | 新增 `towing/towing_env_cfg.py`、`scripts/towing/tow_drag.py`，扩充 `utils/recording.py`（`TowRecorder`）。**关键实现选择**：① **力臂交给仿真**——`set_external_force_and_torque(positions=...)` 的作用点是连杆坐标系（`articulation.py` 的 `apply_forces_and_torques_at_position` 真正消费），故直接传挂点常量、由 PhysX 算力矩，不手写 offset×F，也回避 CoM 约定；② 绳数学走 P3 已验证的标量路径；③ 策略抽帧 `decimation = control_dt/dt` 且要求整除；④ 启动时核对关节集合、`base`/`base_link` 唯一、小车四关节四接触。**验证（离线）**：`test_towing_tow_drag.py` 通过，初始挂点间距用**场景常量 + `cart.urdf` 实读挂点反解**校核；接口契约锁住 `write_data_to_sim()` 早于 `sim.step()`、失败路径 `os._exit(2)` 且不调用 `application.close()`（CART-02 的教训）、场景只改 `prim_path`、入口不导入 `velocity_env_cfg`/`amp_env_cfg`。**测试抓到两个真实缺陷**：① `--damping 0`/`--slack 0` 被「必须为正」的校验错拒；② 在 `InteractiveScene(...)` **之后**才写 `robot.init_state.pos`，而场景构造时就按 init_state 摆资产 ⇒ `--spawn-height` 会被静默忽略。详见 [P4 下半记录](docs/towing_p4_tow_drag_2026-09-20.md) |
| 2026-09-20 | 按计划 **P4 上半**：冻结底层策略的接口契约与适配器，登记 TOW-02 | 新增 `towing/utils/policy_cfg.py`（纯标准库契约）与 `towing/utils/low_level_policy.py`（TorchScript 适配器）。契约按部署侧权威来源对齐，关键语义：`last_action` 是**裁剪后**动作（部署 `obs.actions = Forward()`）、关节目标 `= default_dof_pos + action_scale*clip(action)`、观测拼接后整体 clip ±`clip_obs`；`reset` 契约 `last_action` 归零 + `reset_settle_s`（初值 1.0 s，待仿真确认）。**验证（离线，本机无 GPU）**：`test_towing_policy_contract.py` **20 项通过**，其中最关键的是**直接读 `imgo2_deploy/policy/imgo2/amp/config.yaml` 与 `base.yaml` 逐项交叉核对**（观测项顺序、缩放、动作缩放、裁剪界、默认姿态、`joint_mapping`、控制周期），任一不一致即失败 ⇒ 避免契约在训练侧与部署侧各自漂移；适配器用临时 TorchScript 假策略做端到端验证（观测组装顺序/缩放/裁剪、动作裁剪、`last_action` 回灌、`reset`、维数自检报错、缺文件报错）。全量离线测试 **142 → 162 项通过**，三个校验脚本退出码 0。**未做/待验证**：未改 locomotion、未接入场景、未跑仿真或训练；`reset_settle_s` 与绳参数未标定。详见 [P4 上半记录](docs/towing_p4_policy_contract_2026-09-20.md) |
| 2026-09-20 | 按研究计划 **P3 实现绳索力**（单侧弹簧阻尼），并登记 TOW-01 | 新增 `imgo2_rl/source/imgo2_rl/imgo2_rl/tasks/manager_based/towing/mdp/rope.py`：`T = 0（δ≤0）/ max(0, kδ+cḋ)（δ>0）`、`F_R=+T·e`、`F_L=-T·e`，另提供 `rope_extension`、`offset_torque`（力臂力矩）、`point_velocity`（挂点速度 v_com+ω×r，因为 ḋ 必须用挂点速度而非质心速度）；`mdp/__init__.py` 导出公共 API。**按计划明确不做** rope USD / 多刚体绳。**验证（离线，本机无 GPU）**：`test_towing_rope.py` **31 项通过**（松弛/δ=0 边界、绳不能推、单调性、方向与力的模、ḋ 的投影语义、力臂叉乘、参数校验、动量守恒、总能量不增、松弛段动能漂移恰为 0、numpy 批量与 torch 一致、`mdp` 包含导入契约），全量离线测试 **111 → 142 项通过**，三个校验脚本退出码 0，`git ls-files -i -c --exclude-standard` 为空。**两条实测模型性质**：① `δ≤0 ⇒ T=0` 单向成立，反向不成立（δ>0 快速接近时被截断，20000 步中 250 步）⇒ 判据要用 `T>0`；② 自由双质点下松弛段冻结相对速度，ζ=1.01 仍有 −0.085 m/s、97.6% 松弛，**不可外推到 P4**。**未做/待验证**：未接入场景（需 P4 的第二个刚体）、未标定 k/c/L0、未跑仿真或训练。详见 [P3 记录](docs/towing_p3_rope_2026-09-20.md) |
| 2026-09-20 | **P1/P2 验收全部完成**：阻力扫描 8/8 通过，趋势判据成立 | 运行 `16885db6` 串行单独执行成功，8 个 case 全部 `valid=true`、2001 样本、`coast.svg` 齐全、四轮接触 1.0。**停止距离随 b 单调下降**：0.5 m/s 为 1.034 / 0.516 / 0.257 m，1.0 m/s 为 2.111 / 1.053 / 0.524 m ⇒ `distance_decreases_with_damping=true`（两组）。**`b=0` 阴性对照干净**：两个速度下 10 s 内速度只变 3e-5 / 2e-5 m/s、`stopped=false`（符合 §6 预期）⇒ 无虚假拖滞。`calibration.json` 候选 b = `[0.016, 0.032]`（1.0 m/s 停止距离落在 0.5–2 m；0.008 的 2.111 m 超上界被正确排除）。**六个已停止工况逐个对解析黏性模型**（`τ=m_eff r²/(4b)`、`t=τ·ln(v0/0.02)`、`D=v0·τ(1−0.02/v0)`）偏差 **0.27%–0.96%**，且随 b 单调增大、实测恒略小于解析值，与 dt=0.005 的离散化一阶误差一致。**操作前提**：Isaac Sim 须**串行**执行——同 13 秒内并发启动的三条运行（`6ba3cab4` 空、`c2868729` 空、`ef391b97` 只 2/8）都未正常收尾，单独运行的都成功；并发争锁是推断原因（未取到当时控制台输出）。详见 [检查与验收记录](docs/cart_p1_p2_checks_2026-09-20.md) |
| 2026-09-20 | 训练机补充 **P2 dt 复核通过**；阻力扫描中断于 2/8 | **dt 复核通过**：`--dt 0.0025` 对 dt=0.005（同工况 `v=1.0 b=0.016`）停止距离 **1.055570 对 1.053125 m（差 0.23%）**、停止时刻 4.220 对 4.215 s（0.12%）、四轮接触均 1.0、正功均 0.0 ⇒ 接触与停止距离对步长不敏感。**扫描未完成**：运行 `ef391b97` 在第 2 个 case 的 t≈5.87 s 被中断（`experiment.json` 停在 `starting`），只产出 `v=0.5 b=0`（完整：10 s 仅掉 0.003% 速度、`stopped=false` 符合预期）与 `v=0.5 b=0.008`（截断：1175/2001 样本，`v(5.87 s)` 实测 0.03286 对解析 0.0330）；其余 6 个 case 缺失，故 `distance_decreases_with_damping` 仍为 `null`。**推断原因**：同 13 秒内并发启动了 3 个 Isaac Sim 实例且都未正常收尾，而单独运行的那条完成 ⇒ 串行执行；需控制台输出确认。**顺带**：用这个真实截断产物验证了「残缺运行不得重标为有效」——离线重算得 `valid=false`、`failures=["run_not_completed","incomplete_or_irregular_samples"]`、退出码 1（此前只有合成单测）。详见 [检查与验收记录](docs/cart_p1_p2_checks_2026-09-20.md) |
| 2026-09-20 | 在训练机（RTX 4090 容器）执行 **P1/P2 物理验收**：P1 通过，P2 单工况通过 | **P1 落地通过**：`--mode drop --duration 3` → `state=completed`、`valid=true`、四轮 `contact_fraction` 0.9734、静止高度 **0.150000 m**、tilt 9.8e-6、质量 9.99999964 kg、四 wheel joint、刚度/阻尼恰好 0、模型哈希 `866934f6…`。未接触的 2.66% 经核对全是 3 cm 掉落瞬态（`fl_normal_n=0` 恰 16/601，落地 t≈0.08 s）；末步四轮法向力合计 98.04 N ≈ mg，左右 8% 差为静不定接触的求解器分配（`base_link` 惯性原点 `0 0 0`，非模型不对称）。**P2 单工况通过**：`v=1.0 b=0.016` → 1.0→0.00027 m/s、停止 4.215 s、距离 **1.053 m**、接触 1.0、滚动残差 1.2e-4、正功 0.0 J；**决定性证据**是与解析黏性衰减吻合 0.3%（τ 1.0800/1.0769 s、停止 4.225/4.215 s、距离 1.058/1.053 m），逐点 `tau≈-b·ω_上一帧`（2 个表面正功样本在 t≈9.87 s 的微幅换向，按文档口径归零）⇒ 力矩确在 PhysX 生效。**未验证**：阻力扫描与 dt 敏感性未跑，「停止距离随 b 下降」趋势仍未成立。**环境告警**：warp 报 `cuDeviceGetUuid` 驱动入口错误，未阻塞。详见 [检查与验收记录](docs/cart_p1_p2_checks_2026-09-20.md) |
| 2026-09-20 | 按用户要求拉取远端最新（含合并提交 `f3657cc`）后**执行 P1/P2 检查** | **拉取**：`e50179f → f3657cc` 快进（`9be905d` cart 线与 `e50179f` main 线合并）。核对合并本身：main 线 12 个改动文件除 `paper_plan_imgo2.md` 外全部被继承，唯一损失是 `aff4c61` 的删除解掉了删除/修改冲突、把 `13eeb4e` 晚 39 分钟的 416 行草稿改动从工作区去掉，已按 README 记载的「仅从索引移除、本地保留」原样还原（与 `e50179f` blob 逐字节一致，继续被忽略）。**已通过（本机复跑，退出码 0）**：cart 22 项测试、全量 111 项测试、`check_cart_model.py`、`check_asset_paths.py`、`check_model_sync.py`、`run_isaaclab.sh --check`（`dlopen(linalg) OK`）。**受阻（未验证）**：P1 drop 与 P2 coast 均停在 Isaac Sim 物理初始化，`RuntimeError: No CUDA GPUs are available`——容器 `NVIDIA_VISIBLE_DEVICES=void`、无 `/dev/nvidia*`、`torch.cuda.is_available()=False`，Isaac Sim 无 CPU 回退；CART-01 维持待验证，阻力扫描与 dt 敏感性未跑。**新登记**：CART-02（初始化失败退出码为 0，根因是 `finally: application.close()` 先于异常传播、Kit 关停直接终止进程）、CART-03（离线重算入口对失败目录抛 traceback）。未改代码/配置，未提交推送。详见 [检查记录](docs/cart_p1_p2_checks_2026-09-20.md) |
| 2026-09-20 | 为推送 P1/P2 合并远端 `e50179f`，保留 `9be905d` 及双方历史 | **已修/验证**：解决 README 与论文草稿跟踪状态冲突，保留双方维护记录；本地草稿哈希一致并继续忽略，无关规划删除未纳入提交。小车 22 项与训练曲线 6 项离线测试通过；P1/P2 运行命令接入远端新增启动器。**待验证**：CART-01 训练机物理验收，未在 Windows 启动 Isaac Lab。详见 [合并与同步记录](docs/cart_p1_p2_implementation_2026-09-20.md#5-待完成与同步状态)。 |
...
| 2026-09-20 | 修 `gait_metrics` 的 **yaw 解缠缺陷**；判读 rlamp 首次步态回放（用户终端实测） | **缺陷**：`body_attitude_summary` 直接用 `arctan2` 的 yaw（±180° 折叠）算「峰峰」与「漂移率」，航向跨过 ±180° 会凭空多/少 360°。用户摘要里 `yaw 峰峰 359.95°` 与漂移率 `−2.324 °/s`（19 s 才 −44°）自相矛盾即此（24500 那次没跨界所以 24° 是对的）。已改成先 `np.unwrap` 再统计，新增 2 项测试（跨界/不跨界）⇒ 全仓 **89 项通过**；本次摘要的两个 yaw 数需重跑。**回放判读**（`--command_vx 1.0`，8×950 步，与 2026-09-18 测 24500 那版同配置同指令）：周期 **0.193→0.296 s**（离参考 0.600 从 3.1× 缩到 2.0×）、相位 FL-FR **+185.3°**（干净对角步）、无恒定抬头（pitch 均值 −0.63° 对 24500 的 +2.15° 恒定）、步幅 0.14–0.16→0.17–0.19、抬脚 0.067–0.085→**0.139–0.163**（偏高）；但 **跟速 0.108→0.566 m/s**（差 5 倍）、**站高 0.312→0.371 m**、落地集中度 R 0.86–0.91→**0.31–0.76**、**四足占空比之和 2.19→1.48 ⇒ 出现腾空相（跑而非走）**、shank 峰峰 0.52–0.62→0.99–1.09。**待确认的重要假象**：同一 checkpoint 训练日志站高 0.313 m 而 play 回放 0.371 m ⇒ 怀疑策略有「高/低两个稳定站高分支」（训练 85% 从录制帧≈0.297 起，play 从 `init_state` z=0.35 起）；验证办法是回放时保留 `reference_state_initialization` 或把 `init_state.pos.z` 设 0.297 再测。**未验证**：本机无 GPU，未复跑；且用户本次没 `--out`、未确认 checkpoint，按「晚期 checkpoint（20k–24k）」处理。详见 [§18](docs/amp_gait_adjust_plan_2026-09-18.md) |
| 2026-09-20 | **修 `libtorch_cuda_linalg.so: cannot open shared object file`**：新增统一启动器 `imgo2_rl/scripts/run_isaaclab.sh` | 用户跑 `eval_gait.py` 时崩在 `spawn_from_urdf → create_prim → xform_prim.set_world_poses → isaacsim.core.utils.torch.transformations.get_local_from_world → torch.linalg.solve`。**根因（离线已复现）**：① Isaac Lab 的 `SimulationContext` 把 Isaac Sim 的 prim/变换后端硬编码成 `backend="torch"`（`isaaclab/sim/simulation_context.py:269/279`），于是建场景要用 torch 的 CUDA linalg；② 该实现由 torch **延迟 `dlopen`** 的 `libtorch_cuda_linalg.so` 提供，而 `libtorch_cuda.so` 里存的字符串是**裸文件名**（`strings` 已确认），glibc 对显式 `dlopen` **不使用 `DT_RPATH`**（该 so 只有 RPATH、没有 RUNPATH，也没进 ld.so.cache）⇒ 只能靠 `LD_LIBRARY_PATH`；③ 训练那次 shell 里恰好有 `torch/lib`，这次没有 ⇒ 同一个安装、同一份文件，行为不同。**离线复现**（无需 GPU）：`env -u LD_LIBRARY_PATH /opt/conda/envs/isaaclab/bin/python -c "import ctypes; ctypes.CDLL('libtorch_cuda_linalg.so')"` 报出**逐字相同**的错误；把 `<site-packages>/torch/lib` 加进 `LD_LIBRARY_PATH` 后同一句成功。**修法**：启动器把 `<site-packages>/torch/lib` 与所有 `nvidia/*/lib` **追加**到 `LD_LIBRARY_PATH`（追加避免遮蔽系统 `/usr/local/cuda`），并提供 `--check` 自检；用法 `bash imgo2_rl/scripts/run_isaaclab.sh <脚本.py> [参数]`。**验证**：新增 `tests/test_run_isaaclab_launcher.py` **4 项**（可执行位、`--check` 报告 .so 存在且 dlopen OK、子进程 `LD_LIBRARY_PATH` 含 torch/lib 与 nvidia/、裸名 dlopen 负例 vs 走启动器正例）⇒ 全仓 **87 项通过**。**未验证**：沙箱无 GPU，`torch.linalg.solve(cuda)` 那一步只能在有卡的终端确认（`--check` 在有卡时会多打一行 `CUDA solve : OK`）|
| 2026-09-20 | **给平地 rlamp 加回随机持续外力**（`external_force=True`，用户决定） | 上一轮「都对齐 rl_amp」时按参考把 `randomize_apply_external_force_torque` 关成了 `None`（参考 `a1_amp_config.py` 的 `domain_rand` 没有这一项）。用户 2026-09-20 决定**平地版**加回来，取值沿用仓库原来的 `EventCfg`：base 上 `force_range/torque_range = ±10 N / ±10 Nm`、`mode="reset"`；语义是 Isaac Lab 的 `apply_external_force_torque`——**reset 采样一次、整个 episode 持续施加**（写进 `set_external_force_and_torque` 缓冲，每物理步生效），对 5.53 kg 机体 ≈1.8 m/s² ≈0.18 g 的持续扰动。**实现**：`apply_rlamp_env_settings(cfg, *, custom_origins=False, external_force=False)` 新增该开关，`Imgo2AmpRLAmpEnvCfg` 用 `external_force=True`；**粗糙版仍忠于参考（无外力）**；回放/评估由 `apply_amp_play_overrides()` 继续置 `None`（确定性）。**必须知道的后果**：① 这是一次**新的训练条件**——2026-09-18 跑满 24420 轮的那个 run 是**无外力**的（其 `params/env.yaml` 是记录），所以**不要拿加力后的配置去 resume 它**（等于把两种条件混在一起），要另开新 run；② 平地/粗糙这对模板从此**不再只差地形**（粗糙版无外力），要做干净对照就给粗糙版也加（`apply_rlamp_env_settings(self, custom_origins=True, external_force=True)` 一行）。**验证**：`test_amp_rlamp_recipe.py` 扩到 20 项——常量（±10 N/Nm）、helper 源码含外力分支、平地版调用带 `external_force=True` 而粗糙版不带、行为测试（`external_force=True` 时新建 `EventTerm(mode='reset', func=apply_external_force_torque)`、`asset_cfg=('robot', ['base'])`、范围 ±10；默认 `False` 时为 `None`）⇒ 全仓 **83 项通过**。**未验证**：本机无 GPU，未实例化配置、未训练 |
| 2026-09-20 | 按用户要求拉取远端最新提交：**核对结果 = 本地已经是最新，没有需要合并的改动**（未执行 `pull`，工作树一行未动） | **对象与结果**：本机 `HEAD` = `40cb298`（2026-09-18 21:35 +0800「修控制台进度条分母…」），`git ls-remote` 显示远端 `refs/heads/main` = `40cb2983c3dd683efc630cdbfdd678ab75d533ea` ⇒ 逐字相同；`fetch origin '+refs/heads/*:refs/remotes/origin/*' --prune --tags` 退出 0 且**未产生任何新引用**，`git rev-list --left-right --count origin/main...HEAD` = `0 0`（不领先、不落后）、`git status -sb` 无 `[behind]` ⇒ 无需 `merge`/`pull`。**未提交改动原样保留**：`imgo2_rl/scripts/tools/read_tfevents.py`（+40/−2）与 `imgo2_rl/tests/test_read_tfevents.py`（+34/−3）仍是 ` M`，本轮没有 stash、没有覆盖（本次不是它们的提交，其内容与验证状态见后续条目）。**环境事实（本容器 ≠ AGENTS.md 描述的 Linux 主机）**：① 直接访问 `github.com` 不通（20.205.243.166 上 443 超时；对照 `140.82.112.3`/`20.27.177.113` 可通，但 git 走 DNS 得到的 IP）；本地 `7890`/`7897`/`10808` 均**无进程监听**，代理客户端不在本机；② 只有 `gh-proxy.org` 这条路能走（远端本就配置为 `https://gh-proxy.org/https://github.com/qmq-h/imgo2.git`），而仓库是**私有**的（同方式访问公开仓库 `octocat/Hello-World` 返回 `001e# service=git-upload-pack`，本仓库返回 `401 Repository not found`）⇒ 必须带 PAT；③ `credential.helper=store` 在本容器**必然失败**（`~/.git-credentials` 落在只读根文件系统上，报 `无法在 1000 ms 获得凭证存储锁: 只读文件系统`），`HOME=/tmp/githome` + `-c credential.helper=` 才是可用组合；④ 只读根也意味着上一轮配的 `~/.ssh/id_ed25519` **不存在**（`/root/.ssh` 只有 `authorized_keys`），SSH 认证不可用。**可用命令**（凭据取自既有的 `~/.git-credentials`，形如 `https://qmq-h:<PAT>@gh-proxy.org`，下面用变量避免明文入库）：`USERPASS=$(sed -E 's#^https://([^@]+)@.*$#\1#' /root/.git-credentials); git -c credential.helper= -c http.proxy= -c https.proxy= fetch "https://${USERPASS}@gh-proxy.org/https://github.com/qmq-h/imgo2.git" '+refs/heads/*:refs/remotes/origin/*' --prune --tags`（`HOME=/tmp/githome` 与清空 `http_proxy` 等变量是为了绕开 `~/.gitconfig` 里指向已失效的 `gh-proxy.org` 的 `insteadOf` 重写和外部代理变量）。**未做**：未改动任何代码/配置（只改本节与顶部日期）、未创建提交、未推送 |
...
| 2026-09-20 | 修正 `read_tfevents.py` 的 x 轴陷阱：把「秒」当成迭代轮号会虚报进度 | 第一版对**全部 tag 取最大 step** 当作迭代数，但 rsl_rl 的 `Train/mean_reward/time`、`Train/mean_episode_length/time` 两个 tag 用的是 `tot_time`（**墙钟秒**）当 x 轴，数值比轮号大约 ×1.25 ⇒ 读出来 `last_step=24420` 而真实是 24000 轮附近也无法区分（早先还把 4391 当成轮号报给用户，实际当时约 3500 轮）。现在表头改印 `iterations=<取自迭代轴 tag>`（优先 `Train/mean_reward`）并对带 `/time` 后缀的 tag 单独提示，表格里每个 tag 仍按各自 x 轴取值。**验证**：`test_read_tfevents.py` 加 1 项（合成「迭代轴 step=80 / 时钟轴 step=100」的文件，断言表头取 80 并提示时钟轴）⇒ 全仓 **83 项通过**；并实测该 run 的真实轮号与 `model_*.pt` 的 `iter` 字段一致 |
| 2026-09-18 | **修冒烟训练崩溃**：`apply_rlamp_env_settings()` 对 `None` 事件取 `.params`（`AttributeError`），并把 `reference_state_initialization_prob` 对齐到参考的 0.85 | 用户实跑 `--task=Imgo2-basemove-flat-amp-rlamp` 时在**环境构造阶段**崩：`cfg.events.randomize_reset_base.params[...]` → `'NoneType' object has no attribute 'params'`。**根因**：`Imgo2AmpMoveEnvCfg.__post_init__`（`amp_env_cfg.py:155-156`）把 `randomize_reset_base`/`randomize_reset_joints` 设成 `None`（AMP 的 reset 交给参考状态初始化）⇒ 只能**新建 EventTerm**，不能就地改值。**离线测试没抓到，是因为我手写的鸭子类型 cfg 把这两项填成了对象**（前置条件与真实配置不一致），已改成照抄真实覆盖（`None`）。**修法**：① helper 改为新建 `EventTerm(func=mdp.reset_root_state_uniform/reset_joints_by_scale, mode='reset')`；② `reference_state_initialization_prob` 1.0 → 参考的 **0.85**（否则参考初始化会覆盖掉那 85%、新建的 reset 分布白写）；③ `apply_amp_play_overrides()` 把这两项重新置 `None`（回放要确定性初始状态；其它 AMP 变体本来就是 None，行为不变）。**顺序前提**：`EventManager` 按 `cfg.__dict__` 顺序跑 reset 项（`event_manager.py:337`，不排序），`reference_state_initialization` 是子类字段 ⇒ 最后跑；新建事件必须用 `EventCfg` 里已有的字段名，拼错的新键会被追加到最后而反转顺序（已写成测试守住）。**验证**：测试 73 → **76 项通过**，新增 3 条防回归（源码交叉检查「就地改 vs 基类置 None」、reset 项执行顺序不变量、标准库 dataclass 复现`_custom_post_init` 的顺序机制）。**未验证**：需用户重跑冒烟训练确认能构造环境并跑满 100 轮（本机无 GPU，无法实例化配置类）。详见 [§15.2.1](docs/amp_gait_adjust_plan_2026-09-18.md) |
| 2026-09-18 | 按用户要求「**都对齐，除了关节 kp/kd**」把 rlamp 两个任务的**指令范围 + 域随机化 + reset 分布 + 观测噪声 + `min_normalized_std`** 与 rl_amp 逐项统一 | 新增 `apply_rlamp_env_settings(cfg, *, custom_origins=False)`（`amp_env_cfg.py`）与 `AMPRLAmpRunnerCfg`（`agents/amp_rsl_rl_cfg.py`），四个 rlamp 任务（含 play）改指向新 runner。**改动**：`lin_vel_x [-1.0,2.0]`、`lin_vel_y ±0.3`；摩擦 static/dynamic 都 `(0.25,1.75)`、restitution `(0,0)`、base 质量 `(-1,1)` kg；**关掉** 其它 body 质量缩放 / CoM 偏移 / 外力力矩（参考 `domain_rand` 里根本没有这些字段——它不继承基类，这一点已由测试直接读源码验证）；PD 增益 `×U(0.9,1.1)` 且 `mode` 由 `reset` 改 `startup`；推力 `±1.0 m/s` / 每 `15 s`；reset 平地不加 xy/偏航扰动、粗糙 xy `±1 m`、根速度 6 维 `±0.5`、关节角 `default_dof_pos×U(0.5,1.5)`；actor 噪声 `dof_pos 0.01→0.03`、`ang_vel 0.2→0.3`；`min_normalized_std [0.01]*12`（`clamp_noise_std=False` 下不生效，仅保持可追溯）。**代码里写明的三点**：① `ang_vel` 噪声在参考里是**死代码**（它的 actor 切掉线/角速度，`legged_robot_amp.py:269`），我们保留 `base_ang_vel` 所以有效；② **没改**共享地形材质（`restitution=1.0` + `multiply`）——机器人 restitution 被显式写成 0，净值为 0，改它会牵动全部任务与已部署策略；③ 对齐 = **减少**随机化，鲁棒性下降，且 `lin_vel_x` 上限 2.0 m/s 超出录制数据覆盖的 0.842 m/s。**仍未对齐**：actor 45 vs 42（`base_ang_vel`，待用户决定）、kp/kd（用户排除）、`IMGO2_CFG` 里的求解器/接触参数（全任务共享，不动）、AMP 数据来源。**验证**：`test_amp_rlamp_recipe.py` 新增 6 项，其中两项是实质验证——**把 helper 的 AST 抽出来喂鸭子类型 cfg 真跑一遍**、**直接读参考源码**比对 commands/`domain_rand`/`noise` 与 reset（正则抓 `torch_rand_float` 端点）⇒ 全仓 **73 项通过**。**未验证**：未训练（Isaac Lab 配置类在沙箱无法实例化）；详细对照表见 [§15.2](docs/amp_gait_adjust_plan_2026-09-18.md) |
| 2026-09-18 | 按用户要求把 **critic 缩放 / commands 缩放 / 动作缩放+裁剪** 与 rl_amp 统一（只作用于两个 rlamp 任务） | 新增 `apply_rlamp_obs_and_action_scales()`（`amp_env_cfg.py`），在 `Imgo2AmpRLAmpEnvCfg` 与 `Imgo2AmpRoughEnvCfg` 里与 `apply_rlamp_task_rewards()` 一起调用。**改动**：① critic 观测缩放 `lin_vel 2.0 / ang_vel 0.25 / dof_pos 1.0 / dof_vel 0.05`（我们原来继承 Isaac Lab 默认全 1.0 ⇒ 真实偏差）；② **commands 缩放**改成参考的 `commands_scale = [2.0, 2.0, 0.25]`（`legged_robot.py:515`；Isaac Lab 的 `ObsTerm.scale` 支持元组，`observation_manager.py:551-558` 会校验长度）——**这条同时动到 actor 输入尺度**（用户只点名 critic，我按"统一"一并处理，改回只需一行）；③ 动作 `action_scale = 0.25` 统一（原髋 0.125）、`clip = ±100`（原 ±3）。**部署契约影响（后续必做）**：这三个任务的策略将来导出到 `imgo2_deploy/policy/imgo2/amp/` 时，`config.yaml` 必须同步 `action_scale`（全 0.25）、`clip_actions_lower/upper`（±100）、`commands_scale`（[2,2,0.25]）——都是 yaml 字段，C++ 不用改；**本轮刻意不改那份 yaml**（它服务已部署的 24500 策略）。**验证**：`test_amp_rlamp_recipe.py` 新增 3 项（helper 覆盖 policy+critic、critic 额外 lin_vel；**直接读参考基类源码**比对 `obs_scales`/`clip_actions` 与 a1 的 `action_scale`；其它变体不得被动到）⇒ 全仓 **65 项通过**。**未验证**：未训练（配置类在沙箱无法实例化） |
| 2026-09-18 | 核对 **actor/critic 与 rl_amp 的一致性**（用户提问）：网络一致、critic 一致，**actor 差 `base_ang_vel` 这 3 维** | **依据**（`rl_amp/.../base/legged_robot_amp.py:250-271`）：参考把 48 维 privileged obs 拼好后执行 `if self.num_obs == self.num_privileged_obs - 6: self.obs_buf = self.privileged_obs_buf[:, 6:]`，**切掉前 6 维 = 线速度 + 角速度** ⇒ 它的 actor 是 **42 维**（`projected_gravity + commands + dof_pos + dof_vel + actions`，**既不看线速度也不看角速度**）；我们是 **45 维**（AMP-05 只去掉线速度、保留 IMU 角速度）。**一致项**：网络 `[512,256,128]+elu+init_noise_std 1.0`、critic **48 维且组成与顺序完全相同**、共同观测项的缩放（ang_vel 0.25 / dof_pos 1.0 / dof_vel 0.05）。**不一致项**：actor 多 3 维；critic 各项缩放我们是 Isaac Lab 默认全 1.0（参考 2.0/0.25/1.0/0.05）；执行层差异（action_scale 髋 0.125 vs 统一 0.25、clip ±3 vs ≈不裁剪、PD kp 25 vs 20——机器人不同，不应逐字对齐）。**决定（建议并记录）**：**保持 45 维**，因为 `base_ang_vel` 是部署契约里的项（改它要同步 `amp/config.yaml` + C++ 观测拼接），且真机 IMU 本来就有；本轮检验的是配方（2 项奖励 + coef/lerp），观测集差异留作独立单变量。**未验证**：无（本条是源码核对，不需要运行） |
| 2026-09-18 | 粗糙版加**特权 height scan**（critic 48 → 235，actor 仍 45） | 用户决定「就给粗糙版加」。`Imgo2AmpRoughEnvCfg` 恢复 187 维 `height_scanner`（`GridPatternCfg(0.1, [1.6,1.0])` → 17×11）并**只**接到 critic（`observations.critic.height_scan = ObsTerm(clip±1, scale 1.0)`）；`observations.policy.height_scan` 保持 `None` 并在代码里 `assert` 守住 ⇒ **actor 仍 45 维、部署契约（`amp/config.yaml`、C++ 接口）不变**；判别器吃 `observations.amp`（43 维）不受影响；critic 235 正好等于 amp_go2 的 `num_privileged_obs`。**代价**：粗糙版比平地版多一个变量（"critic 是否看地形"），平地对粗糙不再是严格单变量对照（要做干净对比需给平地版也加，或做"粗糙 − 特权 critic"消融）。**验证**：新增/改写 3 项测试（特权 scan 只进 critic、从 `GridPatternCfg` 反算网格维度并断言 critic=235、三个平地版不得加 scan）⇒ 全仓 **62 项通过**。**未验证**：Isaac Lab 配置类在本机沙箱无法实例化，尚未跑过训练 |
| 2026-09-18 | **新增粗糙地形版 AMP** `Imgo2-basemove-rough-amp`（配方不变，只改地形相关四处） | **做法（含同日更正）**：新增 `Imgo2AmpRoughEnvCfg` / `Imgo2AmpRoughPlayEnvCfg`（`amp_env_cfg.py`）并注册 `Imgo2-basemove-rough-amp-rlamp` + `-play`（用 `AMPRunnerCfg`）。**最初写成继承平地 6 项配方，按用户要求改为「忠于 rl_amp」：任务只留线/角速度跟踪（每步 1.0 / 0.3333），其余全 0**，并新增成对的平地版 `Imgo2-basemove-flat-amp-rlamp`（同一配方）⇒ 平地对粗糙是单变量对照。除配方外，只有四处**因地形而必须**的改动：① 恢复 `ROUGH_TERRAINS_CFG` 生成器 + `terrain_levels` 课程，子地形范围**沿用仓库已有 PPO rough 任务**（boxes 0.025–0.1 m / stairs 0.025–0.08 m / noise 0.01–0.06，机器人站高仅 0.30 m，Isaac Lab 默认太陡）；② 恢复 9 射线 `height_scanner_base`（**只给奖励**）并把 `base_height_l2` 改成**地形相对**；③ `mdp.amp_root_z` 新增可选 `sensor_cfg`（默认 None ⇒ 平地行为不变），粗糙版传同一扫描器 ⇒ AMP 观测最后一维变成"离地形高度"，与专家数据同口径；④ **关掉参考状态初始化**（它写参考动作的绝对根高、无地形补偿，`amp_events.py:60-61` 只给 x/y 加 env_origins，在抬高格子上会初始化到地面以下）。**刻意不做**：不把 187 维高度网格加进 actor（保持 **45 维盲走**、部署契约不变）、不引入 `feet_air_time`/`collision` 等 go2 任务项、不动域随机化（与平地版和 a1 原仓库一致，摩擦/质量/质心/PD/外力都开着）。**验证**：新增 `tests/test_amp_rough_recipe.py` **9 项** AST 契约（地形/课程恢复、子地形范围与 PPO rough 一致、高度奖励与 AMP 根高均为地形相对、参考初始化已关、actor 仍盲走、配方未被改动、注册齐全），全仓 `python3 -m unittest` → **52 项通过**。**未验证**：Isaac Lab 配置类在本机沙箱无法实例化（`import omni.log` 失败），**一次都没跑过**；另外本仓库的 rough 任务（含 PPO rough）**从来没有在本机实际运行的记录**，`height_scanner_base` 对生成地形的射线命中（`mesh_prim_paths=["/World/ground"]`）也只按上游惯例配置，需短训练确认；blind + 单帧无历史在粗糙地形上更难，预期需要更多轮次 |
| 2026-09-18 | **移植 amp_go2 配方**为新任务 `Imgo2-basemove-flat-amp-go2`（用户要求「尽可能参考 amp_go2」）；旧 AMP-only 任务原样保留 | **做法**：新增 `Imgo2AmpGo2StyleEnvCfg` / `Imgo2AmpGo2StylePlayEnvCfg`（`amp_env_cfg.py`）与 `AMPGo2RunnerCfg`（`agents/amp_rsl_rl_cfg.py`），注册 `Imgo2-basemove-flat-amp-go2` 与 `-play`；把 `-play` 的公共覆盖抽成 `apply_amp_play_overrides()` 供两个 play 配置共用（行为不变）。**奖励逐字照抄参考**（两边 `weight` 语义相同，都会被乘 dt）：`tracking_lin_vel 4.0`、`tracking_ang_vel 2.0`、`lin_vel_z −1.0`、`ang_vel_xy −0.05`、`dof_acc −2.5e-7`、`torques −1e-4`、`base_height −1.0`、`action_rate −0.01`、`collision −1.0`（只惩罚 thigh）、`dof_pos_limits −2.0`、`feet_air_time 1.0`；runner 用 `amp_reward_coef 0.2` / `amp_task_reward_lerp 0.8`（风格上限 0.04/步）。**两处有意偏离**（已写进代码注释）：`base_height` 目标 0.30 m（参考 0.38 是 Go2 站高）、`feet_air_time` 阈值 0.2 s（参考硬编码 0.5 s，等于奖励周期 ≥1 s，比我们参考的 0.600 s 还慢）。**验证**：新增 `tests/test_amp_go2_recipe.py` **9 项**（纯标准库 AST 契约），其中一项**直接读本机参考项目 `~/Desktop/AMP/amp_go2-main` 的 `class scales` 逐项比对**（数值一致、无漏抄项）；另校验 11 个项名真实存在于 `RewardsCfg`、重建项的 `SceneEntityCfg` 正则非空（`""` 会在构造环境时报错）、每步换算等价（`weight_il × step_dt == raw × dt_lg`）、runner 权重、任务注册且旧任务未被改动。全仓 `python3 -m unittest` → **40 项通过**；三项离线检查 exit 0。**未验证**：Isaac Lab 配置类无法在无 Isaac Sim 的沙箱里实例化（`import omni.log` 失败），**没有跑过任何训练**。**风险（重点盯）**：`base_height` 每步系数从 −5.0 降到 −0.02（弱 250 倍），我们自己的 Run2 正是弱高度项下退化成贴地滑行（0.172 m）⇒ 短训练里必须看 `AMP/mean_root_height_m` 与 `fraction_root_height_below_0_20m`，若爬行就把 `base_height` 单独调回（单变量）；回报量级缩小约 12 倍，`Loss/value_function` 应从 ~29 降到 ~1–2 |
| 2026-09-18 | **修正前几版回放的口径错误**（三条结论撤回）＋ 把「风格占比不够」证伪为「风格项没有梯度」 | **口径修正（v3）**：加 `--warmup=50` 并修掉 `joint_pos` 堆叠轴后重跑（`--command_vx=1.0/0.6`，用户终端），实测 **步频 0.1926 / 0.1980 s（5.19 / 5.05 Hz；参考 0.600 s ⇒ 3.1 / 3.0 倍）**、相位 **FL-FR +174.2°、FL-RR +17.3°、集中度 0.86–0.91**（与 Gazebo 176.9° 相差 3° 内）、关节峰峰值 **shank 0.516–0.618 / thigh 0.407–0.470 rad（落在参考范围内）**、抬脚 0.067–0.085 m、前后腿比 1.06–1.13（参考 0.87–1.06）、pitch 恒定 **+2.09° / +2.87°**、高度 0.3117 / 0.3041 m。⇒ **撤回三条 v1 结论**（高抬腿 1.6 倍、前后腿不对称 1.33、shank 幅度超参考上限）：前两条是**出生瞬态污染**，第三条是 **`joint_pos` 堆叠轴 bug**；**唯一实质缺陷收敛为「步频 3.1 倍 + 恒定抬头」**，且 0.6 m/s 在数据覆盖内也照样 3.0 倍 ⇒ 「指令超范围」不是原因。**判别器分析**：新增 `probe_amp_policy_vs_expert.py`（策略侧，CPU）与 `probe_gp_effect.py`；策略 `d=−0.145` vs 专家 `+0.572`，把「关节角+足端+关节速度」三块换成专家可补回 **86%** 的风格差距（**joint_vel 单独 53%**；策略 joint_vel 的 σ 是专家 **1.7 倍**），而 joint_pos/foot_pos 的边缘分布几乎重合（σ 比 1.00）⇒ 判别器抓到的是**时间尺度**而非姿态；**但策略点局部梯度仅 8e-5**（1σ 全维扰动只移动 d≈5e-4）⇒ 风格奖励是没有方向的台阶，故**单纯加权重无效**。合成对照（正类=1×帧对、负类=3×帧对，同预算 1500 步）`λ_gp` 10/1/0 的 d 跨度为 0.149/0.812/1.799、风格奖励差 0.105/0.564/1.262 每步 ⇒ 指向 **λ_gp=10 压扁判别器输出幅度**（边界：合成问题、预算更小，只支持趋势不支持绝对值）。**调整优先级**：P1 降 λ_gp、P2 加 `feet_air_time`（目标滞空 ≈0.2 s）、P3 兜底 `lerp 0.3→0.5`、P4 抬头用 `flat_orientation_l2` 消融。**验证**：31 项单测通过、三项离线检查 exit 0、两次 GPU 回放（用户终端）+ 三份离线探针。**未做**：改训练配置、重训、真机/GUI/MuJoCo |
| 2026-09-18 | 用**离线判别器探针**回答「是不是风格占比不够」：不是占比，是**风格项没有梯度** | **回报分解**（读 24500 轮 event 文件）：每步混合回报 1.97 = 任务 1.402（71%）+ 风格 0.540（29%）；`d_expert` 全程停在 +0.55~+0.66、`d_policy` 停在 −0.42~−0.66，5000 轮后风格项就在 0.44–0.56 打转，而速度误差同期 1.11→0.468 ⇒ **任务项有方向、风格项近似常数**。**探针**（新增 [probe_amp_discriminator.py](imgo2_rl/scripts/tools/probe_amp_discriminator.py)，只读 checkpoint 的 `discriminator_state_dict`+`amp_normalizer`，用 rl_lab 自己的 AMPLoader 取专家数据，CPU 可跑）实测：真实专家 d=+0.571，**全部输入置零**仍 +0.527（几乎无输入依赖），**按 k 倍速播放专家动作**（k=3 模拟实测步频倍数）d 只掉到 +0.504 ⇒ 混合后风格项只少 **0.013/步**；而全维加 1σ 噪声 d 掉到 **−0.095**、2σ 掉到 **−0.771**；归一化空间局部梯度合计 **1e-4**（流形内部平坦），trunk 输出范数 3.29、ReLU 活跃率 0.50/0.43（不是死网络）。⇒ **判别器实际是「在不在录制流形上」的探测器**，策略无法靠改善步态把 d 推上去（风格项可达上限 1.335/步 vs 现状 0.540，但唯一有效方向是「更像录制数据的整体分布」，含仿真↔录制的域差）。**结论**：不建议单纯加大 `lerp`/`coef`；优先级为 ①指令范围压到数据覆盖内 ②单变量试 `lerp 0.3→0.5` ③修退化（λ_gp=10 是压平流形内部的嫌疑项）④最后才加显式步频项。**同轮工具修正**：eval_gait 的 `joint_pos` 堆叠轴写反（关节峰峰值全报 0.000）已修并加 `_expect()` 形状校验；相位改用「插值到相邻两个参考落地之间」（v2 的取模法集中度只有 0.15–0.17，因 0.1927 s ÷ 0.02 s 不是整数步而累积误差）；新增 `--command_vx/--command_yaw`（扫速用）与 `--dump-npz`（把策略侧 43 维 AMP 观测落盘，之后判别器分析可完全离线）；`--warmup=50` 揭示 **v1 的「高抬腿 1.6 倍」「前后腿不对称 1.33」是出生瞬态污染，两条结论撤回**（真值 0.066–0.083 m 和 1.10）。测试 31 项全过（新增 4 项：转置输入必须报错、漂移周期下的插值相位、末步超时 vs 窗口内重置、逐环境聚合自洽）。**未做**：策略侧 AMP 观测的离线分析（需一次带 `--dump-npz` 的 GPU 回放，沙箱内 PhysX 无 CUDA，已在 `--device=cpu` 上实测确认无 CPU 回放路径）；未改任何训练配置、未重训、未跑真机/GUI/MuJoCo |
| 2026-09-18 〔**注意：本条里「抬脚高度 1.6 倍」「前后腿不对称 1.33」「关节幅度超参考上限」三条结论已被下方同日期的更正条撤回——见「修正前几版回放的口径错误」**〕| 24500 轮的**足端步态回放**（训练侧 Isaac Lab）：独立复现 Gazebo 结论、量化剩余差距、修掉评估脚本两处测量缺陷 | **跑法**：`python scripts/tools/eval_gait.py --task=Imgo2-basemove-flat-amp-play --checkpoint=logs/amp_rsl_rl/base_move_amp/2026-09-17_21-25-57/model_24500.pt --num_envs=8 --steps=1000 --out ../docs/gait_eval_amp_24500.json --headless`（解释器 `/opt/conda/envs/isaaclab/bin/python`；play 配置把指令固定为 `vx=1.0 / vy=0 / yaw=0`）。**结果**：高度 0.3122±0.0056 m、线速度误差 0.1163 m/s、角速度 0.0959 rad/s；**步周期 0.193 s（参考 0.600 s ⇒ 快 3.1 倍）**、步幅 x 0.151–0.219（均 0.179，同速档参考均 0.295 ⇒ **0.61 倍**）、抬脚 z 0.172–0.183（参考 0.109 ⇒ 1.6 倍）、shank 峰峰 1.389–1.432（参考中位 0.635、上限 1.063）、thigh 约 1.8 倍、占空比 0.50–0.61、机身 pitch **恒定 +2.13°**（参考前进 −0.5°）、roll 0.21°、yaw 峰峰 17.7°/20 s；**后腿步幅比前腿大 1.33 倍**（用标准库重算 4 个前进档，参考是「前≥后」，同口径 base 系）。自洽性：`0.179×5.15 = 0.92 m/s` ≈ 实测 `1.0−0.116 = 0.884 m/s` ⇒ 指标可信，剩余问题定性为**步幅偏小 + 抬腿偏高，靠高步频补速度**。**修掉两处测量缺陷（v1 JSON 相关字段作废）**：① 逐环境占空比原写成 `c[:, e].mean()`（对同一时刻的各环境求均值，得 k/8 的离散值，**看起来像「7/8 环境的 RL 足整段不落地」**）——实为索引错轴，改成 `c[e].mean()` 并加自洽断言；② 相位原只取「0 号环境 + 首个落地事件」（±0.1 周期量化），改成全部事件 × 全部环境的**圆周平均 + 集中度 R**，并把 FL-FR 相位写进摘要。另外更正一处旧误读：`episode_length_mean_steps = 499.5` 是 `0…T−1` 的**斜坡均值**，不含重置信息，v2 改记 `done` 计数与末步缓冲值，并加 `--warmup`（默认 50 步，丢弃出生 0.35→0.31 m 的下落瞬态）；`shank_peak_to_peak_rad` 旧名其实含 12 个关节，新增 `joint_peak_to_peak_rad` 按腿展开。**口径下沉**：新增纯 numpy 的 [gait_metrics.py](imgo2_rl/scripts/tools/gait_metrics.py)，`eval_gait.py` 只采样与打印；新增 [test_gait_metrics.py](imgo2_rl/tests/test_gait_metrics.py) **18 项**（合成 trot 端到端跑 `build_report`），`python3 -m unittest discover -s tests -p "test_*.py"` → **27 项全通过**；`check_asset_paths/check_model_sync/check_amp_joint_order` 仍 exit 0。**限制**：两个新 bug（参考足无落地时的 `IndexError`、相位「步 vs 秒」单位混用）都是**合成测试**抓到的，不是实跑；v1 那次回放没有 `--warmup`、也没有重置计数，且其相位/逐环境列不可用 ⇒ **需要用修好的脚本重跑一次（约 2 分钟，需用户在终端执行）**，本次未做。**沙箱内代跑已实测不可能**：`--device=cpu` 能过参数解析、建环境、载 checkpoint（顺带给 `AMPOnPolicyRunner.load()` 补了 `map_location=self.device`，见下）和读 21 份 AMP 动作，但 Isaac Sim 的 PhysX 必须有 CUDA（`Failed to create any GPU devices, including an attempt with compatibility mode`），不存在 CPU 回放路径。`AMPOnPolicyRunner.load()` 的 `map_location` 是在这次尝试中补的：GPU 同设备重映射按构造恒等，但未在 GPU 上回归 ⇒ 「已修，待验证」；PPO/HIM runner 有同样写法，本次未动。未跑真机/GUI/MuJoCo，未重训。详见 [足端步态回放记录](docs/gait_eval_amp_24500_2026-09-18.md) |
| 2026-09-18 | 确认 **24500 轮** AMP checkpoint（`model_24500.pt`）并接入部署；低速与站立达标 | **对象**：`policy/imgo2/amp/model_24500.pt`（sha256 `cc8ee22a…`）+ 其导出 `policy.pt`（`cba59d44…`，769312 B）。**契约**：① 导出件 vs checkpoint actor，128 组随机 45 维输入 **max diff 0.0**；② 部署自带 libtorch 2.3.0 加载通过、同探针输出与 Python 一致到 1e-6；③ Gazebo 按「每条命令一条新栈」逐点复测。**结果（跟速）**：0.2 → 93.4%、0.3 → 97.9%、0.5 → **95.9%**（5000 轮时 76%）、0.75 → 98.7%、1.0 → 99.8%、1.2 → 99.8%、1.5 → 98.5%；高度 0.277–0.300 m。**站立**：`vx=0` 漂移 0.047 → **0.001 m/s**、yaw −3.9 → −1.9°/s、抖动 hip/thigh/shank 35.7/39.8/80.1 → **8.6/6.1/9.3**（5000 轮那个「站着不动」的退化解消失）。**步态**：周期强度 0.17–0.20 → **0.71–0.83**（键 1 参考策略 0.96–0.97，已同量级），FL-FR 相位在 0.3–1.5 m/s 全部落在 **174°–188°**（trot ±180°），连 5000 轮偏离大的 0.5（123°）、1.5（156°）都归位；同速度 roll 由 6.8–9.2° 降到 3.5–6.0°。**仍未达标（不夸大）**：步频约为录制参考的 **2.5 倍**（周期 0.208–0.365 s 对 0.600 s / 1.67 Hz），抖动绝对值随速度上升（shank 57→412，1.0 m/s 以上比 5000 轮略高，但同期步频也更快、强度 0.83 相位正确）；极低速 0.2 m/s 的周期强度仅 0.23。**顺带**：该 checkpoint 的 `iter` 字段为 **24500**（不再是 0）⇒ 训练侧 AMP-08「修复后尚未产生新 checkpoint」这条未验证项得到实证。**未做**：`eval_gait.py` 足端层面回放（训练机）、GUI、真机；记录见 [Gazebo 记录 9.7 节](docs/gazebo_ros2_bringup_2026-09-17.md) |
| 2026-09-18 | 评审用户本地重写的 `paper_plan_imgo2.md`（未知绳索负载下的启停瞬态控制），出 [评审记录](docs/paper_plan_review_2026-09-18.md) | 用户本地那份规划已从「Imgo2 小论文选题」整体重写为 **quadruped–rope–passive wheeled load 的启停瞬态 command shaping**（本地未提交，评审未改动它）。**结论**：方向成立、milestone 顺序正确、自我定位诚实（不宣称首次考虑负载惯性/slack-taut——那两点已被 [Yang et al. 2022 多机 cable-towed load](https://ar5iv.labs.arxiv.org/html/2206.14424) 与 cable-trailer 混合动力学+规划工作占据；最近还有 [MULE](https://export.arxiv.org/pdf/2505.00488v1) 的未知负载适应，但其负载在机体上而非绳索外拖）。**新意可站住的是**：单机 + 轮式被动负载（breakaway/滚阻是主导非线性）+ command→0 的卸载瞬态 + 冻结底层只学 command shaping。**用户随后决定：不用拉力计、完全本体感知**（评审记录 §3.1/§3.2/§3.4 已按此重写）——$T,v_L,$slack/taut 全部不可测，而且**机体线速度 $v_R$ 今天也没接**（真机只订阅 lowcmd/lowstate/wirelesscontroller，`obs.lin_vel` 恒 0）⇒ 上层需自己做腿部里程计或订阅 `rt/sportmodestate`。于是重心从"控制"移到"可观测性 + 估计"：主基线改为**跟踪亏欠在线自适应 ramp**（可部署、无学习），张力反馈降级为"模拟张力传感器"的**参照上界**（用来量化不用拉力计的代价）；估计器建议 跟踪亏欠代理 → **浮基动量观测器** → 学习式隐估计（配线性探针），并给出三个可直接做的实验：**breakaway 的阶跃检测**（本体感知最亮的信号，可作启动段相位切换）、估计器噪声/延迟消融、停车段松弛/追尾只能间接判断。**另两个必补**：① 补经典反馈调节器基线（否则"为什么用 RL"必被问）；② 把"必要性"写成**预先注册的定量判据**（单一最优固定 ramp 相对每负载最优的损失阈值，或安全-响应 Pareto 前沿随负载平移）。**物理细节风险**：$T_{\rm peak}$ 主要由绳刚度 $k$ 与步长决定（需 $k,c$ 敏感性 + 步长准则）、滚阻 sgn 需静摩擦分支与 Stribeck 正则化、4 固定轮的横向刮擦需 phase-1 约束成直线或改 2 万向轮、缺机器人自身牵引极限 $\mu$（建议先做 $F_{\rm req}/(\mu N)$ 可行性区域图）、Go2 还是 Imgo2 必须二选一（若用 Imgo2，低速度段恰是实测最弱区：0.5 → 76%、$v=0$ 漂 0.047 m/s）。**另指出一个规划没写但很可能是最好现象的点**：停车后负载追尾（松弛余量不足以吸收负载残余动量 ⇒ cart 撞机器人），应加"最小间距/松弛余量消耗/再绷紧冲量"指标。**本仓库可衔接**：底层策略可用包线（已有实测）、参考步态基线、部署侧 Gazebo 验证台（可扩成记录 $T,v_R,v_L,\theta,D_{\rm stop}$ 的版本）。**未做**：修改用户的规划文件、任何绳索/台车建模与仿真 |
| 2026-09-18 | 拉取远端 4 个提交（AMP-08、判别器诊断、步态工具+基线、`max_iterations` 40000），并同步更新本机文档；**用户本地的 `paper_plan_imgo2.md` 改动原样保留** | **拉取**：`5b84b36 → e64acaa`，`git merge --ff-only`（本机无冲突）。新增/改动：`b3db915` AMP-08 修 checkpoint 迭代号缺陷（AMP/PPO 循环内不同步 `current_learning_iteration`，实测该 run 20 个 checkpoint 全为 `iter=0`，`--resume` 会从 0 重跑——这正好解释了我此前记录的 `model_5000.pt` 里 `iter: 0`）、`7805587` 判别器 expert/policy 原始打分入 TensorBoard、`9380e91` 新增 `imgo2_rl/scripts/tools/play_reference_motion.py` 与 `docs/gait_reference_baseline.json`、`e64acaa` 把 `max_iterations` 10000→**40000**。**本地文件保护**：拉取前把 `paper_plan_imgo2.md`（用户本地修改）备份到 `imgo2_deploy/build/local_backup/`，拉取后校验 sha256 仍是 `7f24432f…`、`git status` 仍显示 ` M paper_plan_imgo2.md` ⇒ **未被覆盖**（本次提交也未把它加入暂存）。**据新提交同步的文档**：AMP 报告 §11.2/§11.3 的 `max_iterations` 改为 40000 并把该优先级标为已完成、`b53ffeb` 注明远端现为 `7805587`；Gazebo 记录 §9.1 注明 `iter: 0` 的原因（AMP-08）、§9.6 增补**与参考步态基线的对照**（录制动作 19 份行走周期中位数 **0.600 s / 1.67 Hz** 且与速度无关、只调幅度；AMP 在 1.0/1.2 m/s 是 0.294/0.213 s，即快 2–2.8 倍 ⇒ 数字上支持"高频倒腿凑平均速度"，目标是把周期往 ~1.7 Hz 靠）。**未做**：新工具（`eval_gait.py`、`play_reference_motion.py`）的回放（需训练机 GPU） |
