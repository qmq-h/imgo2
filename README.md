...
| 日期 | 变更 | 验证与限制 |
|---|---|---|
| 2026-09-20 | 按计划 **P4 下半**：机器人+小车+绳的拖曳场景与实验入口，TOW-02 收口为「已实现待训练机」 | 新增 `towing/towing_env_cfg.py`、`scripts/towing/tow_drag.py`，扩充 `utils/recording.py`（`TowRecorder`）。**关键实现选择**：① **力臂交给仿真**——`set_external_force_and_torque(positions=...)` 的作用点是连杆坐标系（`articulation.py` 的 `apply_forces_and_torques_at_position` 真正消费），故直接传挂点常量、由 PhysX 算力矩，不手写 `offset×F`，也回避 CoM 约定（实测 base 惯性原点偏移 <3 mm）；② 绳数学走 P3 已验证的标量路径（单环境）；③ 策略抽帧按契约 `decimation = control_dt/dt` 且要求整除；④ 启动时核对关节**名与顺序**、`base`/`base_link` 唯一、小车四关节四接触。**验证（离线）**：`test_towing_tow_drag.py` **16 项通过**，其中初始挂点间距用**场景常量 + `cart.urdf` 实读挂点反解**校核（间距必须 = L0+slack）；接口契约锁住 `write_data_to_sim()` 早于 `sim.step()` 且晚于施力、失败路径 `os._exit(2)` 且不调用 `application.close()`（CART-02 的教训）、场景配置只改 `prim_path`、入口不导入 `velocity_env_cfg`/`amp_env_cfg`。全量离线测试 **186 项通过**，三个校验脚本退出码 0，`--help` 只用标准库。**训练机第一次实跑暴露并修掉一个关键问题**：Isaac Lab/PhysX 的 DOF 顺序按运动学树**广度优先**（全部 hip → 全部 thigh → 全部 shank），而 `urdf/imgo2.urdf` 是逐腿声明、AMP 任务又把策略的观测与动作显式设成逐腿顺序 ⇒ 两者**必须显式置换**。第一版直接比较两者并要求相等，启动即失败；若改成「把契约顺序改成广度优先」来消错，会让观测拼错、动作打到错误关节（静默且致命）。修法：`policy_cfg.asset_permutation()` 比集合+建置换，入口重排关节量进观测/动作换回资产顺序，并用「资产 `default_joint_pos` 重排后须等于契约 `default_dof_pos`」做强校验；置换写入 `config.json`。回归用**实跑报告的真实顺序**做固定样例（`JointOrderTests` 5 项）。**训练机第二次实跑又暴露并修掉一个集成缺陷**：运行目录被两处重复创建——入口用 `mkdir(exist_ok=False)` 独占建目录后，`TowRecorder` 又照抄 `CartRecorder` 的「自己建目录」语义建了一次 ⇒ `FileExistsError`（自己撞自己）。已改为「目录由入口独占创建、recorder 只写入且拒绝覆盖已有产物」，并补 3 条测试覆盖这条集成路径（离线单测此前只单独构造 recorder，没覆盖入口流程）。**顺带清理**：删掉无运行时消费者的 `apply_joint_mapping`（其部署侧语义已被 `asset_permutation` 取代，两个相似 API 并存正是上次搞混的温床；`joint_mapping` 字段保留并仍与部署 yaml 交叉核对）。**测试另抓到并修掉两个真实缺陷**：① 第一版把 `--damping`/`--slack` 放进「必须为正」的校验循环，导致 `--damping 0`（无阻尼绳）与 `--slack 0`（初始绷直）被拒；② 第一版在 `InteractiveScene(...)` **之后**才写 `robot.init_state.pos`，而场景构造时就已按 init_state 摆好资产 ⇒ **`--spawn-height` 会被静默忽略**，已改为构造前写配置并加测试锁住先后顺序。**未验证**：全部 Isaac Lab/PhysX 调用只做过静态核对，物理行为待训练机；绳参数与 `--spawn-height` 未标定；质量扫描与 PPO 契约未做。详见 [P4 下半记录](docs/towing_p4_tow_drag_2026-09-20.md) |
