...
> 最后核对：2026-09-20（P1/P2 验收全部通过；**按计划 P3 实现单侧绳力、P4 实现冻结策略契约+适配器+拖曳场景与入口**，均通过离线验证——全量离线测试 178 项、三个校验脚本退出码 0；P4 的物理验收待训练机执行，命令见 [P4 下半记录](docs/towing_p4_tow_drag_2026-09-20.md)）。本文是整个工作区的维护入口，覆盖项目总览、训练部署流程、已知问题和变更记录。
...
| TOW-02 | P1 | **P4 已实现（契约+适配器+场景+拖曳入口），离线验证通过；物理验收待训练机** | 按计划 P4「先完全不改 locomotion」：① `towing/utils/policy_cfg.py`（AMP 45 维契约：`base_ang_vel×0.25 3` + `projected_gravity 3` + `velocity_commands 3` + `joint_pos_rel 12` + `joint_vel_rel×0.05 12` + `last_action 12`，`clip_obs 100`、动作缩放 `(0.125,0.25,0.25)×4`、裁剪 ±3、默认姿态 `(0,0.87,-1.82)×4`、控制周期 0.005 s；纯标准库）② `utils/low_level_policy.py`（观测组装 → TorchScript 推理 → 裁剪动作 → `default+scale*action`）③ `towing/towing_env_cfg.py`（机器人+小车+接触场景，只改 `prim_path`，不碰增益/观测/奖励）④ `scripts/towing/tow_drag.py`（每物理步绳力+轮阻 → 写仿真 → step → 记录；策略抽帧按契约）⑤ `utils/recording.py` 增 `TowRecorder`。**动作语义按部署 C++ 对齐**：`last_action` 是**裁剪后**的动作、关节目标 `= default_dof_pos + action_scale*clip(action)`；**力臂交给仿真**（`set_external_force_and_torque(positions=...)` 是连杆系作用点，由 PhysX 算力矩，不手写 `offset×F`）；绳数学走 P3 已验证的标量路径。**验证**：`test_towing_policy_contract.py` **24 项**（含直接读部署 yaml 逐项交叉核对防漂移、5 项关节顺序回归）+ `test_towing_tow_drag.py` **20 项**（初始挂点间距用场景常量与 `cart.urdf` 实读值反解校核、参数校验、记录器、接口契约：`write_data_to_sim()` 必须早于 `sim.step()`、失败路径 `os._exit(2)` 不调用 `application.close()`）⇒ 全量离线 **162 → 178 项通过**，三个校验脚本退出码 0，`--help` 只用标准库。详见 [P4 上半](docs/towing_p4_policy_contract_2026-09-20.md)、[P4 下半](docs/towing_p4_tow_drag_2026-09-20.md) | 训练机**串行**执行 `bash scripts/run_isaaclab.sh scripts/towing/tow_drag.py --velocity 0.5 --duration 5 --headless`，按 `summary.json` 的 `steady_*`/`tension_*` 判「稳定拖曳」。**已跑通流程但行为尚失败**：初始挂点间距被算成 1.4777 m（设计 0.95）、初始张力 **1955 N** 把机器人 0.1 s 内拽到 pitch −0.74 rad 并拽倒 ⇒ 修正「挂点位置漏加偏移」与「`--slack` 写成预张紧」两处 bug，并把判据加上 `steady_tracking_ratio`（两体都静止时原判据会误通过）与 `tension_slack_fraction`；按 `summary.json` 的 `steady_*`/`tension_*` 判「稳定拖曳」（`robot_vx≈load_vx≈v_user`、张力正且波动 < 25%、绳长略大于 L0）；待确认 `--spawn-height`（0.35 vs ≈0.297 的高/低站高分支）与绳参数 k/c/L0；后续节点：质量扫描（缩放质量须同时缩放惯量）、PPO 底层契约、P5 步态类 Logger |
