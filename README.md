...
| 日期 | 变更 | 验证与限制 |
|---|---|---|
| 2026-09-24 | **相位核变陡**（`std` 0.7071→**0.2**、`max_err` 0.2→**0.5**）＋新增两个**接触时序诊断项** | 依据 §29.31：run G 实测 `trot−bound` 只有 −0.010 且第 50→436 轮完全平坦 ⇒ 相位项边际奖励仅 0.01/s，等于没在塑形。**改动四处同参**：`feet_gait`（`CMoE_env_cfg.py` post_init 覆盖，原先一直沿用原版 PPO 的 `std=√0.5`/`max_err=0.2`）＋三个 `gait_metric_{trot,bound,pace}`（类体）⇒ 单核地板 `exp(−2·0.2²/0.7071)=0.893` → `exp(−2·0.5²/0.2)=0.082`，参考步态（T 0.93 s、duty 0.5）trot 1.000 / bound 0.302 ⇒ 溢价 **0.264 → 0.698/s（2.7×）**，且非 trot 步态不再拿 0.74。**新增 1e-6 纯记录项**：`diag_air_time`（四足 `last_air_time` 均值＝`ā`）与 `diag_pair_mismatch`（六对 `|Δair|+|Δcon|` 均值）⇒ 逐列 `gait_airtime_<地形>` / `gait_mismatch_<地形>`，用来分辨"长步幅慢步 vs 短步碎步"（`feet_air_time` 只约束乘积 `N落地·(ā−0.5)`，分不开这两者）并按**实测**定 `max_err`。生效奖励 25→**27 项**。**验证**：新增 `test_contact_timing_diag.py`（5 例：`ā` 均值、lockstep⇒0、trot 快照⇒0.4、六对覆盖、`body_ids` 生效）；`test_check_reward_overrides.py` 改为 27 项＋新增"四处同参"断言（**负向对照**：只把 `feet_gait` 的 std 改回 0.7071 ⇒ 该用例失败）；全仓离线 **349 通过 / 8 跳过**。**未验证**：重启后的实际效应（要看 `gait_trot_*` 是否开始压过 `gait_bound_*`、`gait_airtime_*`/`gait_mismatch_*` 的实测值）。详见 [训练准备记录](docs/cmoe_training_prep_2026-09-23.md) §29.32 |
| 2026-09-24 | run G 首查（436 轮）：**NaN 已除、课程分档见效，但步态不是 trot 且相位项没在起作用** | run G＝`2026-09-24_21-23-48_cmoe_G_77dim_fresh`（4096 环境，≈4.4 s/轮）。**修复确认**：132 tag 零 NaN，逐列 `tracking_*`/`gait_*_<地形>` **353/353 轮都有点**（§29.30 的修法在真实训练里成立）。**健康**：速度核 0.200→**0.617**（峰值 0.656@250）、`ep_len` 705→885、`illegal_contact` 0.274→0.260、`noise_std` 稳 0.94、`lr` 2.3e-3→2e-4、`tracking_mean` 0.144→**0.721**、`progress_mean` −0.47→**+9.39 m**、`level_mean` 1.14→1.93。**课程分档见效**：同龄(300–356) `pyramid_stairs_inv` **0.10→2.93**（run C 到 1000 仍卡 0.30）、`pyramid_stairs` 0.56→3.72；代价是 0.8 档四类同龄偏低（`flat` 2.91 vs 3.58、`gap` 0.61 vs 1.05）＋摔率 0.265 vs 0.172 ⇒ 砍腾空付款＋加 vz 罚的直接代价，非 bug。**步态判读（本轮重点）**：三分类器 trot **0.9266**／bound **0.9364**／pace 0.9265（8 列同型，bound 全列最高）⇒ **不是 trot**；`trot−bound = −0.010±0.002` 从第 50 轮到 436 轮**完全平坦**（期间速度核 0.20→0.62）⇒ **相位项对步态毫无影响**（它付钱买的就是这个差值，差值不动＝没起作用）。**为什么**：`GaitReward` 单核 `exp(−clip(Δ²,max=0.04)/0.7071)` ⇒ 单核区间 **[0.893, 1]**、6 核乘积 **[0.508, 1]** ⇒ 时间差一超 `max_err=0.2 s` 就贴地板，度量近似**二值**（"有几对配对"）；实测 0.010 的差≈**0.1 个核**⇒ 边际奖励仅 **0.01/s**（总量 ~5.5/s 的 0.2%），PPO 不会动。**离线按同一公式**算参考步态（T 0.93 s、duty 0.5）：现参数下 trot 1.000 vs bound 0.736（溢价 0.264/s）；`max_err 0.5/std 0.2` 下 1.000 vs 0.302（溢价 **0.698/s**，2.7×）。**对照补充**：四足 `last_air_time` 散布仅 **RMS 0.078 s**（`feet_air_time_variance` −8.0 反解）⇒ 步态是"短而整齐、四足近同步"⇒ 短步子让**所有**核对都≈1、同时喂饱三个度量（也解释了 `feet_air_time` 只约束 `N_落地·(ā−0.5)≈−190`、`ā` 与落地率不可分辨）。**结论**：现相位项更像"短而整齐步子"的塑形，不是 trot 塑形 ⇒ 继续训练不会长出 trot。**建议（待用户定）**：①`feet_gait`＋三度量 `max_err 0.2→0.5`、`std 0.7071→0.2`（溢价 0.70/s）；②`feet_air_time` 0.3 是否回调待"接触时序诊断"数据；③新增 1e-6 诊断项记 `ā` 与六对 `|Δair|+|Δcon|`（纯记录）；④相位压力要在 **400–1000 轮**加上（run C 1000 轮已定型且在蹦）。**未验证**：以上均为离线读 run G 的 tfevents＋按源码公式复算，未重启验证。详见 [训练准备记录](docs/cmoe_training_prep_2026-09-23.md) §29.31 |
