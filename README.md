...
| AMP-05 | P1 | **训练侧已产出 45 维 checkpoint 并接入部署（2026-09-18）；待换成 `play.py` 正式导出、且 5000 轮尚不会走** | actor 应移除 `base_lin_vel`。**2026-09-18**：拿到训练侧 45 维 AMP checkpoint `amp/model_5000.pt`（第一层 `[512,45]`，critic 仍 48 维），已把 `amp/config.yaml` 改成 `num_observations: 45` + `observations` 去掉 `lin_vel`，并换上新导出的 `amp/policy.pt`；Gazebo 无头实测键 3 可进入、站姿 0.27–0.28 m、`vx=0.5` 走 0.151 m/s（旧 48 维那份只 0.057 m/s），四项指标见 [Gazebo 记录](docs/gazebo_ros2_bringup_2026-09-17.md) 第 9 节 | ① 把"导出走 `play.py`（headless）"这条落实：训练侧需先把 45 维配置（`observations.policy.base_lin_vel = None`）推入仓库，否则 `play.py` 会在 `load_state_dict` 尺寸不匹配；② 用 `play.py` 重新导出并按三条契约复核（本机无 NVIDIA 驱动跑不了 Isaac Sim，本次用的是等价复刻导出）；③ 步态仍不对（周期强度 0.21、shank 抖动 120）⇒ 回到 AMP-07 的奖励/终止修法并继续训练。**不得直接截掉旧网络的 3 个输入。** 详见 [sim2sim 记录](docs/sim2sim_amp_2026-09-17.md) |
