...
| 小车 MuJoCo sim2sim | 两个拖曳 MJCF 场景已接入现有 `rl_sim_mujoco`，5–15 kg 质量与惯量同比例缩放；URDF 与 MJCF 静态契约检查通过 | 需在 MuJoCo 环境编译、加载场景并完成牵引／停止回放；上层策略尚无可部署 checkpoint，Gazebo 小车场景尚未实现，见 TOW-05 |
...
bash imgo2_rl/scripts/run_isaaclab.sh imgo2_rl/scripts/rl_lab/towing/play.py \
    ...
    --num_envs=1 --headless \
    --checkpoint="logs/towing_rl_lab/towing_upper/<run>/model_2000.pt" \
    --episode-log --cmd-interval=50 --cmd-summary --decoder-summary --max-steps=2000
```

`--num_envs=1` 约有 12.5% 概率抽到「无小车」环境，要统计量就开 `--num_envs=32`；`--max-steps` 以控制步计（`step_dt=0.05 s`，2000 步 ≈ 10 个回合）。汇总默认写到 checkpoint 同级的 `play_summary.json`，其键的判读口径、各档指令（看画面／录视频）与一次该 run 的 TensorBoard 读数见 [拖曳回放指令与前置核对](docs/towing_play_2026-09-25.md)。
...
**2026-09-25 新入库的拖曳占位（未接线、不可加载）**：`policy/imgo2/towing/policy.pt` 是
`logs/towing_rl_lab/towing_upper/2026-09-23_22-19-00/model_2000.pt` 的**原样拷贝**
（sha256 `a4d800a26c1d1a50…`，9.76 MB），里面是**训练态** checkpoint（actor／critic／decoder 三套 GRU
权重 + 两个优化器 + critic normalizer + `iter`），**不是** `play.py` 导出的 TorchScript，
`inference_runtime` 的 libtorch 加载器读不了它；该目录也**没有** `config.yaml`，上表三个键之外
**没有**对应 FSM 状态类。它只表示"权重已留档入库"（用户 2026-09-25 决定），
不代表部署链路可用——真正的导出件（51 维帧 + decoder／actor 双 GRU hidden 的 TorchScript）
仍需给 `towing/play.py` 加导出路径后在训练机 headless 跑一次，见 TOW-05。
...
# 小车拖曳：默认 10 kg，--cart-mass 可设 5–15 kg；仅这两个场景接受该参数
./cmake_build/bin/rl_sim_mujoco imgo2 scene_tow_compliant --cart-mass 5
./cmake_build/bin/rl_sim_mujoco imgo2 scene_tow_inextensible --cart-mass 15
```

两个拖曳场景都包含 `cart.xml`（与 `cart/cart.urdf` 同尺寸／惯量）、四轮轴承阻尼和 0.8 m 单边绳长约束；默认机器人在前、小车在后，绳初始松弛 0.4 m。运行 `python imgo2_rl/scripts/tools/check_towing_mjcf.py` 可离线核对模型参数。现有键 `3` 的 AMP 可用于底层牵引场景试跑；上层 51 维 recurrent 策略仍无已验证导出件，因此完整上层拖曳 sim2sim 尚未闭环。MuJoCo 原生约束与 Isaac Lab 的外力／冲量算法不能直接视为数值等价，见 [拖曳 sim2sim 记录](docs/towing_sim2sim_2026-09-24.md)。
...
| TOW-03 | P0 | **自有 recurrent PPO＋decoder runner 已接线；任务已注册；4 环境冒烟（2026-09-22）、4096 环境 × 2000 轮完整 run（2026-09-23/24）与确定性回放（2026-09-25）均跑通；跨环境隔离、STOP 排序与 estimator 精度未完成** | decoder 使用 `51→128→GRU(128)` 预测机器人机体系 `vx/vy`、负载质量和牵引力 `Fx/Fy`，5 维 estimate detach 后组成 56 维 actor 输入。仓库 `rl_lab` runner 已管理 decoder／actor／critic 三套 GRU、critic-only normalizer、rollout estimate 固化、PPO 后 decoder 更新及联合 checkpoint；不依赖外部 RSL-RL runner/config API。STOP 后有界 GT 拉力惩罚已加入。**2026-09-22 训练机前置核查**：拖曳离线测试 210 项全通过、51 维帧与 56 维 actor 契约一致、冻结 AMP 策略与 `torch.jit.load` 路径齐备；修掉 `train.py` 的 `--agent` 默认值（原 `towing_rl_lab_cfg` 全仓无对应注册项，必抛 `ValueError`）；加入注册 `Imgo2-towing-upper-rl-lab`（`--agent=rl_lab_cfg_entry_point`），并把原守门断言换成锁定注册接线的契约测试（已用两组负向测试确认有效）。详见 [GRU 记录](docs/towing_gru_design_2026-09-22.md) 与 [训练前置记录](docs/towing_training_prep_2026-09-22.md) | 在 Isaac Lab 2.2.1 训练机完成 wrapper 构造、4／256 环境 recurrent rollout、checkpoint 恢复、estimator 精度、STOP reward 排序和无小车隔离验证，重点排除策略为卸载拉力而让小车逼近的捷径。**注意**：注册是用户为执行训练而显式翻过 `towing_upper_rl_review` 第 98 行「未完成运行验收前不注册」这道保护，验收通过前该项不得记为完成。**2026-09-22 冒烟实跑（4 环境 × 10 轮）通过**：环境构造成功、51 维帧／56 维 actor 契约在运行时成立（`memory_a` 输入 56 维、`memory_c` 65 维）、checkpoint 含三套 GRU＋decoder＋两优化器＋critic-only normalizer、`iter` 正确写入 0→10（AMP-08 修好后首次在真实 checkpoint 上确认）、decoder 可从 checkpoint strict 加载并推理。详见 [冒烟记录](docs/towing_smoke_run_2026-09-22.md)。**2026-09-24 完整 run（用户执行）**：`logs/towing_rl_lab/towing_upper/2026-09-23_22-19-00/`，4096 环境 × 2000 轮跑满（`seed=1`、`save_interval=100`），`model_2000.pt` 的 56 维 actor／65 维 critic／3 维动作与运行时断言一致。TensorBoard 已记录全部分项与诊断：`obs_cart_present=0.8745`≈7/8（12.5% 无小车设计成立）、终止占比 `time_out=0.9882`／`cart_collision=0.0118`／`robot_fall=0`、`tracking_velocity=0.9211`、`yaw_heading` 折合 RMS 1.4°（不再自转）、`min_clearance` 几乎不触发；`obs_towing_force≈obs_towing_force_active`（Σ‖F‖≈328 N·步/回合）说明有拉力时都 >1 N。**2026-09-25 确定性回放（用户执行，`--num_envs=1`、2000 步 = 10 回合跑满）**：`err_cmd=0.339`、`err_track=0.387`、`err_low=0.065` m/s ⇒ `err_track/err_cmd≈1.14`、`err_track/err_low≈5.9`，即误差**仍主要来自上层积分指令没到位**（`ref` 平均比 `user` 少 0.34 m/s），而下层把上层的 `ref` 执行到 0.065 m/s；相比旧 run（不同 checkpoint／旧 `play.py`）的 `err_cmd=0.498`／`err_low=0.097` 有改善但未消除。**该缺口不是"抖"**：`ref` 是 20 Hz 限速率积分器（每步 `x∈[-0.05,+0.025]`、`yaw∈±0.05`），高频抖动上限仅 0.025 m/s；训练日志里 `action_rate`／`action_magnitude` 的数字几乎全部由探索噪声解释（`2Σσ²=0.477` vs 实测 `E‖Δa‖²=0.429`，`std` 取自 checkpoint），因此"抖动仍明显、继续提权这两项"的判断已作废（详见 [记录](docs/towing_play_2026-09-25.md) §7.2）。缺一次 `--cmd-interval=1` 的逐帧 trace 才能区分"起步滞后"与"稳态缺口"。decoder：速度 MAE **0.033 m/s**、力 MAE 0.356 N、质量 MAE 1.62 kg（仅 5.4% 的步受监督，质量项证据薄）。**仍未验证**：跨环境隔离、STOP reward 排序、单环境以外的统计（需 `--num_envs=32`）、checkpoint 恢复、scripted policy 复现；上层策略仍无 `exported/policy.pt`，且 `towing/play.py` 没有导出分支。详见 [回放记录](docs/towing_play_2026-09-25.md) |
...
| TOW-05 | P0 | **MuJoCo 场景已修，待动态验证；完整 sim2sim 未闭环** | 新增 5–15 kg 小车、四轮阻尼、弹性／不可伸长两种单边绳场景，静态契约通过；尚未编译或运行 MuJoCo，且上层策略尚无可部署导出件，Gazebo 小车场景未接入。详见 [拖曳 sim2sim 记录](docs/towing_sim2sim_2026-09-24.md)。**2026-09-25 入库占位权重（用户决定）**：`imgo2_deploy/policy/imgo2/towing/policy.pt` 是 `logs/towing_rl_lab/towing_upper/2026-09-23_22-19-00/model_2000.pt` 的原样拷贝（sha256 `a4d800a26c1d1a50…`，9.76 MB，含 optimizer／decoder／normalizer 的**训练态** checkpoint），**不是** `play.py` 导出的 TorchScript，libtorch 加载器读不了；该目录**没有** `config.yaml`，FSM 也**没有**对应按键／状态类。`towing/play.py` 的导出分支目前为 0 处（ppo／amp／himloco 才有）。详见 [回放记录](docs/towing_play_2026-09-25.md) §9 | MuJoCo 编译／加载与 5、10、15 kg 拖曳停车回放通过；训练侧 checkpoint 经 `play.py` 导出并接入 51 维双 GRU 观测／控制链；Gazebo 场景实现并用相同案例对照速度、停车间隙和碰撞 |
...
| 2026-09-25 | 拖曳上层 `model_2000.pt` 确定性回放实跑（用户执行） | 环境在 15:36:07 重建 USD 缓存后启动，`--num_envs=1`、`--max-steps=2000`（= 100 s 仿真 ≈ 10 个回合）**跑满**，摘要 15:39:36 落盘 `logs/towing_rl_lab/towing_upper/2026-09-23_22-19-00/play_summary.json`。**结果**：`err_cmd=0.339`、`err_track=0.387`、`err_low=0.065` m/s；`err_track/err_cmd≈1.14` ⇒ 剩余误差仍在上层积分指令（`reference_tracking` 方向对但没修完），下层对 `ref` 的执行误差已降到 0.065 m/s。decoder 速度 MAE 0.033 m/s、力 MAE 0.356 N、质量 MAE 1.62 kg（受监督步占比仅 5.4%）。**限制**：单环境单次采样（12.5% 无小车设计下也可能抽到异常工况），`towing_force_gt_mean_N` 是摘要时刻的瞬时值（本次读到 0.0）**不能**当作平均绳力；无视频、无 `exported/`。与旧 run 的对比不同 checkpoint／不同 `play.py` 版本，只作量级参照 |
| 2026-09-25 | 拉取 `main` 到 `962094f`，给出拖曳上层 `model_2000.pt` 的回放指令并核对 run 前置 | `git fetch` 后 `--ff-only` 快进；拉入的 9 个文件只涉及 MuJoCo／C++ 部署侧与文档，不触碰 Isaac Lab towing 回放链路。本地 5 个未提交文件（奖励定义、`velocity_tracking_exp` 改用 `reference_command`、`--compact-log`）保持原样，并经该 run 启动时落盘的 `params/env.yaml` 逐项对照确认**就是这次 run 用的那份**（回退会加载得动但 reward 分项与训练记录对不上）。离线复核：`imgo2_rl/tests` **297 passed**、`check_towing_mjcf.py` OK、`check_model_sync.py` PASS、`check_asset_paths.py` PASS；`model_2000.pt`（`iter=2000`）为 56 维 actor／65 维 critic／3 维动作，与 `TowingVecEnvWrapper` 的 `policy==51 / decoder==6` 断言一致。回放指令三档（headless 诊断版／看画面／录视频）与 TensorBoard 全量读数写入 [记录](docs/towing_play_2026-09-25.md)，README §4.3 补了拖曳 play 示例。**该轮未执行**（回放随后于同日 15:36 由用户跑通，见上一条）：`play.py` 回放与 `exported/policy.pt` 导出——agent 会话进程 `cuda available: False`（无 `/dev/nvidia*`、`NVIDIA_VISIBLE_DEVICES=void`，属 TOW-04 的进程视角），必须在训练机终端运行；本轮未改任何项目代码 |
...
| 2026-09-24 | `main` 增加 5–15 kg 小车 MuJoCo 拖曳场景 | CMoE 分支未提交改动先保存在 stash，随后切到 `main`；加入两类单边绳约束和启动时同步缩放质量／惯量的参数。Python 3.11 标准库静态契约检查、`compileall`、`git diff --check` 与 tracked-ignore 检查通过；本机无 MuJoCo／C++ 构建依赖，未编译或回放。完整策略与 Gazebo 闭环仍待 TOW-05。详见 [记录](docs/towing_sim2sim_2026-09-24.md) |
