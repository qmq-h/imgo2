...
| AMP-07 | P0 | 已定位差异（待改配置重训验证） | AMP 策略「站着不动」的对照排查（见 [AMP 对照报告](docs/amp_standstill_diagnosis_2026-09-17.md)）。**① `clamp(min=0)` 不是差异**：两份参考（`~/RL/isaac/AMP/AMP_for_hardware-main`、`amp_go2-main`）的风格奖励公式与我们逐字相同、都有地板，`d` 为裸 logit。**② 判别器 loss 也相同**（MSE±1、`0.5(expert+policy)`、grad_pen 只对 expert λ=10、共用 Adam lr 1e-3 + trunk/head 不同 weight_decay、`min_std` clamp）。**真正差异在任务奖励量级（③④）**：参考 a1 的 AMP 任务只有速度跟踪两项且每步权重 **50 / 16.7**、其余全 0；我们等效只有 **1.0 / 0.3**（小 50 倍，因为 Isaac Lab 的 RewardManager 会乘 step_dt，把 `1/step_dt` 抵消），却额外保留 **base_height_l2 −10/步**。速度误差 1.65 m/s 时 `exp(-(1.65/0.5)^2)≈2e-5` 梯度≈0，高度项梯度把策略按在「精确站 0.30 m」的局部最优 → 风格分近乎常数（风格≈0.467/步）→ PPO 优势≈0 → 1000 轮后全指标变平。**⑤ 探索与参考逐字相同**（`init_noise_std 1.0`、`min_normalized_std [0.05,0.02,0.05]*4`、`entropy_coef 0.01`），AMP 观测组成也相同（均不含 commands）。`amp_go2`（能走）另用 `coef 0.2 / lerp 0.8`（任务主导） | 已给最小改动：① `track_lin_vel_xy_exp.weight = 50/step_dt`、`track_ang_vel_z_exp = 17/step_dt`；② 去掉 `base_height_l2`；③ 可选照 `amp_go2` 用 `coef 0.2 / lerp 0.8`；④ 加指令课程。改后重训并按「高度/速度误差/关节幅值是否随训练变化」复查（需训练环境，本机无 GPU） |
