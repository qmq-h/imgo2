...
| [imgo2_description/](imgo2_description/) | **模型唯一源（2026-09-17 起）**。`xacro/core.xacro` 是物理内核（17 link/16 joint，命名与训练侧一致 FL/FR/RL/RR），`xacro/robot.xacro` 是组装入口，按开关拼上 `transmission.xacro`（仅 ros_control）、`gazebo.xacro`（插件 + per-link 接触参数）、`imu.xacro`（imu_link）。生成物入库、勿手改：`urdf/imgo2.urdf`（纯）、`urdf/imgo2.gazebo.urdf`（+三者）。`meshes/` 与训练/部署共用同一套（指纹 `8dc5b5995a11`）。`mjcf/` 下的 `imgo2.xml` 是参考项目原版（base 惯量为旧值），deploy 用的场景仍在 `imgo2_deploy/robot_description/imgo2_mjcf/scene.xml` |
...
| AMP-03 | P0 | Torch 回归已通过，待训练中的判别器检查 | 原均值方差更新使用归一化值，梯度惩罚使用未归一化值；已与 `amp_go2-main` 的处理方式对齐。**2026-09-17 在 Linux 机的 `isaaclab` 环境（Python 3.11.14 + torch 2.7.0+cu128，CPU）跑 `tests/test_amp_alignment.py`：6 项全过，其中 `test_raw_statistics_and_normalized_gradient_penalty` 就是这项 Torch 回归**（此前在 Windows 侧因缺 torch 被跳过，是「4 通过 1 跳过」） | 仍待检查新训练中的判别器与归一化统计（需训练环境） |
...
| MODEL-02 | P0 | 阶段 ①③ 完成并验证，②④⑤ 进行中 | 用户决定把模型统一到 `imgo2_description`（D1 命名 FL/FR/RL/RR；D2 Gazebo/IMU/transmission 留在 description 作可选模块；D3 生成物入库；D4 删冗余副本、git 兜底；D5 Gazebo 插件暂不处理）。**已完成**：① `meshes/` 改名为 FL 命名（与其余三份逐字节相同，指纹 `8dc5b5995a11`）；`xacro/core.xacro`（由训练 URDF 抽出，物理权威）＋ `xacro/{transmission,gazebo,imu}.xacro` ＋ `xacro/robot.xacro` 组装入口（开关 transmission/gazebo/imu，默认全 false）；生成并入库 `urdf/imgo2.urdf`（纯）与 `urdf/imgo2.gazebo.urdf`（+transmission 12 +gazebo 21 +base_imu）；删除被取代的 `urdf/imgo2_description.urdf` 与 `xacro/common/`（5 个死文件）；`empty_world.launch` 与 `CMakeLists.txt` 同步；③ `imgo2_description/mjcf/{imgo2.xml,scene.xml}` 由 deploy 那份拆分为「机器人本体 + 世界」（`scene.xml` `<include file="imgo2.xml"/>`），网格路径改 `../meshes/`，追加参考项目有的 `framelinvel`（adr 43，不影响 36-42 的现有偏移） | **验证**：`check_model_sync.py` 五项全通过（4 份已登记 URDF，FK 恒等 RMSE 0.00214 m）；`check_amp_joint_order.py` 三项全过且数值与统一前完全一致（0.00107/0.00214 m、0.22491、0.0191/0.1339 rad、r 0.9546/0.2317）；生成物与训练 URDF **结构等价**（17 link/16 joint 的轴/限位/origin/惯量逐项相同）；`xacro` 重跑与入库文件逐字节一致；负向测试（改一个 hip 轴）能报错且恢复后 sha256 不变；新 MJCF 与旧 deploy 场景的物理回放**逐位相同**（`z_final=0.2621`、AMP 后最高 `0.2907`、`z_min=0.1132`），`mj_loadXML` 结果一致（nq=19 nv=18 nu=12，nsensordata 43→46）。**剩余**：② 消费者切换（RL 的 `assets/imgo2.py`、deploy 的 `build.sh`/launch/`rl_sim_mujoco.cpp` 场景路径）；④ 删除 `imgo2_rl/.../data/imgo2_model`、`imgo2_deploy/robot_description/{imgo2_urdf,imgo2_mjcf}`、`imgo2_model`；⑤ 改写 AGENTS.md 模型章节。详见 [sim2sim 记录](docs/sim2sim_amp_2026-09-17.md) 第 6.6 节 |
