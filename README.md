...
| `Imgo2-basemove-flat-amp-rlamp` | `scripts/rl_lab/amp/train.py`（**rl_amp（fan-ziqi）配方**：任务只留线/角速度跟踪每步 1.0/0.3333，其余全 0；AMP `coef 2.0 / lerp 0.3`，风格约占 2/3；2026-09-18 新增） |
| `Imgo2-basemove-rough-amp-rlamp` | `scripts/rl_lab/amp/train.py`（**粗糙地形 + 同一 rl_amp 配方** ⇒ 与上行成对，地形是单变量；加地形相对根高（仅 AMP 观测）+ 关参考状态初始化；actor 保持 45 维盲走） |
