...
> 最后核对：2026-09-20（P1/P2 验收全部通过；P3 单侧绳力、P4 冻结策略契约/适配器/场景/拖曳入口已实现，且 **P4 核心行为已在训练机验证——0.5 m/s 下 `v_R ≈ v_L ≈ 0.512`、张力 5.2 N 且漂移仅 1.1%、小车被拖 2.47 m**；全量离线测试 199 项、三个校验脚本退出码 0；P4 剩余：1.0 m/s 与 m_L 包线扫描）。本文是整个工作区的维护入口，覆盖项目总览、训练部署流程、已知问题和变更记录。
