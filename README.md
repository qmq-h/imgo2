...
| MODEL-01 | P0 | 已比较，差异属语义问题，待决定基准 | 三份 Imgo2 URDF 的 SHA256 不同。逐项比较结果：**12 个腿部关节的 axis 全部相反**——`Imgo2`（原始片段）与 `Imgo2_rl`（训练）一致为 `hip +1 0 0`、`thigh/shank 0 +1 0`，而 `Imgo2_deploy` 全部取反为 `-1`／`0 -1 0`，关节限位随之镜像（如 thigh `-0.9..2.87` 对 `-2.87..0.9`），这是有意改符号约定而非笔误；基座质量也不同，训练份 `5.53394020` kg、部署份 `3.53394020` kg（差恰好 2.0 kg，占总重约 9.5 kg 的 20% 量级），另有材质块、足端惯量等次要差异，两文件 diff 为 +70/−50 行。**注意 `Imgo2/…/imgo2.urdf` 只是腿部件片段**（无 `base` link、无 `<robot>`），因此它无法作为基座质量的对照基准，但在腿部关节轴上与训练份一致，即部署份是唯一反向者。网格共三套而非一套：`Imgo2/Imgo2_urdf`、`Imgo2_rl/.../Imgo2_urdf`、`Imgo2_deploy/...` **三份共用同一套**（集合指纹 `bc421eb1cbef`，10 文件 14.8 MB 逐字节相同），`Imgo2/imgo2_mjcf` 与 `imgo2_description` 各有独立命名体系（`1ac2f43d08b3`、`24b1aa343fb9`） | 先确定以哪一份关节符号与基座质量为基准（需对照 Gazebo/ROS 与真机 SDK 的实际约定），再据此修正另一份；在此之前不要合并两者的 URDF。基座质量还缺一个可信来源：`Imgo2/` 片段里没有 base link，需回到建模侧确认 5.5339 与 3.5339 哪个是当前真值 |
