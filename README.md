...
| CMoE | 原版 actor/expert/双 estimator/PPO/Storage/Runner 已独立移入，默认保留原版 5 专家；rough 任务已加入 20% 的 `0.10～0.16 m` 沟壑、足端空洞边缘／绊脚奖励、加强的身体碰撞惩罚和 base 触地终止，并移除五项固定步态 shaping；train/play 及 JIT／ONNX 导出已接线，静态编译通过 | PyTorch 张量与 TorchScript 测试、Isaac Lab 环境及四个足端 RayCaster 构造、短训练、checkpoint 回放和实际导出；当前只能确认代码链路，不能声称已训练或收敛 |
...
| CMOE-01 | P0 | **训练／play 代码链路已接齐，待训练环境验证** | `imgo2_CMoE` 分支从原版 CMoE 复制并适配独立 actor/expert/双 estimator/PPO/Storage/Runner；已注册 rough train/play，加入 20% 小沟壑、四足局部空洞边缘 RayCaster、绊脚／碰撞惩罚、base 触地终止和安全 reset 姿态，并补完整 JIT／ONNX 导出。静态编译通过；3 项张量测试因本机 Python 无 PyTorch 跳过，Isaac Lab launcher 也找不到可用解释器。详见 [移植记录](docs/cmoe_port_2026-09-23.md) | 在训练机先确认 4 个足端 RayCaster 的 prim 绑定、gap 未命中和 `feet_edge` 只在接触边缘触发，再以 4 环境、1～2 iteration 验证 observation、rollout、base 终止、gate 与 estimator；之后运行 play 和导出。完成前不得声称已训练或收敛 |
