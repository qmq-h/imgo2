...
> 最后核对：2026-09-20（拉取 `f3657cc` 后执行 P1/P2 检查：P1 落地、P2 单工况、`--dt 0.0025` 复核与 8 case 阻力扫描全部通过，已停止工况与解析黏性模型吻合 1% 以内；**按计划 P3 实现单侧绳力并离线验证通过（31 项测试，全量 142 项）**，接入场景待 P4；另遗留 CART-02/03）。本文是整个工作区的维护入口，覆盖项目总览、训练部署流程、已知问题和变更记录。
...
| TOW-02 | P1 | **P4 上半已完成（冻结策略契约+适配器，离线验证通过）；场景与拖曳入口待做，物理验收待训练机** | 按计划 P4「先完全不改 locomotion」：新增 `towing/utils/policy_cfg.py`（AMP 45 维契约：`base_ang_vel×0.25 3` + `projected_gravity 3` + `velocity_commands 3` + `joint_pos_rel 12` + `joint_vel_rel×0.05 12` + `last_action 12`，`clip_obs 100`、动作缩放 `(0.125,0.25,0.25)×4`、裁剪 ±3、默认姿态 `(0,0.87,-1.82)×4`、`joint_mapping` 恒等、控制周期 0.005 s；纯标准库）与 `towing/utils/low_level_policy.py`（组装观测 → TorchScript 推理 → 裁剪动作 → `default+scale*action`）。**动作语义按部署 C++ 对齐**：`last_action` 是**裁剪后**的网络输出（`obs.actions = Forward()`，而 `Forward()` 末尾就 clamp）。**验证**：`test_towing_policy_contract.py` **20 项通过**（含**直接读部署 yaml 逐项交叉核对**防两处漂移、适配器构造时用零输入自检导出件维数、临时 TorchScript 假策略的端到端检查、契约校验负例），全量离线测试 **142 → 162 项通过**，三个校验脚本退出码 0；并核对真实导出件 `amp/policy.pt` 哈希 `cba59d44e387834e` 与 README AMP-05 一致（该 yaml 头部注释仍写 `model_5000.pt`/`0e5bd661…`，已过期，契约字段未变）。详见 [P4 上半记录](docs/towing_p4_policy_contract_2026-09-20.md) | 做 P4 下半：`towing/towing_env_cfg.py`（机器人+小车+绳场景，沿用 P1/P2 的 `SimulationContext+InteractiveScene` 风格）+ `scripts/towing/` 内拖曳入口；训练机按计划验收 `v_cmd=0.5/1.0 m/s` 的 `v_R≈v_L` 与 `T(t)` 稳定区间，并扫 `m_L` 定 working envelope；PPO 底层契约（与 AMP 不默认相同）留作下一节点 |
