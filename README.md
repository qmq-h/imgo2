...
| CMOE-02 | P0 | **配方已定稿，待训练机从零重训（77 维，不能续训旧件）** | 2026-09-24 用户逐条定稿：① 地形扫描 **77 维**（`scan.resolution=0.1`、`size=(1.0,0.6)` ⇒ 前移 11×7，actor **527**／expert-gate **157**／critic **125**）；② 步态 shaping **照搬那次「trot 还行」的 PPO 三项**并加地形掩码 —— `joint_mirror −1.0`（**含 hip**）、`feet_height_body −5.0`（目标 −0.20 m ≈ 离地 0.10 m）、`feet_air_time +1.0`（**阈值 0.5**），**不使用 `feet_gait`**；`joint_mirror` 为**三态**：trot 地形（13/20 列）用**对角对**、**`gap`（6 列）换成正左右对 ⇒ bound 形状先验**、`boxes`（1 列）完全豁免；`feet_height_body`／`feet_air_time` 在 `boxes`＋`gap` 上豁免。生效奖励 **16 项**。**离线已核对**：`py_compile`、AST 按执行顺序模拟最终权重（`scripts/tools/check_reward_overrides.py`，新增；锁定 `joint_mirror` 曾被晚赋值静默清零的模式）、三个 class-term 的 `__call__` 签名 vs Isaac Lab `manager_base.py:362` 的 `params` 集合校验、新增测试 `tests/test_check_reward_overrides.py` **6 项通过**、`tests/test_masked_joint_mirror.py` **10 项通过**（含"沟壑换成左右对"的负向对照）。**同一处上游陷阱已规避**：`mdp.joint_mirror` 把对子缓存在 env 上且只解析一次 ⇒ 两套对子连调两次会被静默忽略，故改为 `_pairwise_joint_mirror()` 按 term 实例缓存。**同时更正两处旧推理**：`joint_mirror` 的对角对**不该删 hip**（旧注释把对角对误当左右对；parkour 的肩符号翻转是**左右对**的算法，不可类比；残留近似是"对称外展非最小点"，若见歪斜应**翻转 hip 符号**而非删 hip）；`feet_air_time` 的阈值**不改变梯度方向**（`Σ(last_air_time−c)=Σlast_air_time−c·N_落地`，`c` 只改偏移与"减少落地次数"的强度，0.5 是 0.25 的 2 倍）。**已知代价**：去掉 `feet_gait` 后没有任何项区分 trot 与 **pronk**（trot 地形上 `joint_mirror` 能排除 bound／pace，但 `gap` 上它已改成正左右对＝**bound 先验**，而 bound 与 pronk 都满足「两对各自相等」）；依据是 PPO 同配方仍收敛到 trot（部署侧平地实测周期强度 0.95、FL–FR +175°） | 在训练机**从零**新跑（旧 checkpoint 187 几何，尺寸不匹配，**不能 resume／play**）：启动后先确认 `[INFO] num_terrain_obs: 77 / num_obs (total): 527 / num_privileged_obs: 125`，并确认 TB 的 `Episode_Reward/*` 里**有** `feet_air_time`／`feet_height_body`／`joint_mirror`、**没有** `feet_gait`；再回放判读步态（若见对称弹跳：第一顺位恢复 `feet_gait`，第二顺位恢复 `feet_air_time_variance −8.0`）。仍未恢复：`feet_air_time_variance`(−8.0)、`feet_slide`(−0.05)。另：`tests/test_base_height_per_env.py`（5 项，锁定 `base_height_l2` 的逐环境兜底修复）在裸 `python3` 与被 launcher 调起的解释器下**都会 skip**（缺 `omni.log`），必须在**已引导 Isaac Sim 的进程**里跑。详见 [训练准备记录](docs/cmoe_training_prep_2026-09-23.md) §29 |
