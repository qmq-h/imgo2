...
| MODEL-02 | P0 | 阶段 ① 完成并验证，②③④ 进行中 | 用户决定把模型统一到 `imgo2_description`（D1 命名 FL/FR/RL/RR；D2 Gazebo/IMU/transmission 留在 description 作可选模块；D3 生成物入库；D4 删冗余副本、git 兜底；D5 Gazebo 插件暂不处理）。**已完成**：① `meshes/` 改名为 FL 命名（与其余三份逐字节相同，指纹 `8dc5b5995a11`）；`xacro/core.xacro`（由训练 URDF 抽出，物理权威）＋ `xacro/{transmission,gazebo,imu}.xacro` ＋ `xacro/robot.xacro` 组装入口（开关 transmission/gazebo/imu，默认全 false）；生成并入库 `urdf/imgo2.urdf`（纯）与 `urdf/imgo2.gazebo.urdf`（+transmission 12 +gazebo 21 +base_imu）；删除被取代的 `urdf/imgo2_description.urdf` 与 `xacro/common/`（5 个死文件）；`empty_world.launch` 与 `CMakeLists.txt` 同步 | **验证**：`check_model_sync.py` 五项全通过（4 份已登记 URDF，FK 恒等 RMSE 0.00214 m）；`check_amp_joint_order.py` 三项全过且数值与统一前完全一致（0.00107/0.00214 m、0.22491、0.0191/0.1339 rad、r 0.9546/0.2317）；生成物与训练 URDF **结构等价**（17 link/16 joint 的轴/限位/origin/惯量逐项相同）；`xacro` 重跑与入库文件逐字节一致；负向测试（改一个 hip 轴）能报错且恢复后 sha256 不变。**剩余**：② 消费者切换（RL 的 `assets/imgo2.py`、deploy 的 `build.sh`/launch/`rl_sim_mujoco.cpp` 场景路径）；③ MJCF 归位到 `imgo2_description/mjcf/` 并带入训练侧 base 惯量 + 参考项目求解器块；④ 删除 `imgo2_rl/.../data/imgo2_model`、`imgo2_deploy/robot_description/{imgo2_urdf,imgo2_mjcf}`、`imgo2_model`；⑤ 改写 AGENTS.md 模型章节。详见 [sim2sim 记录](docs/sim2sim_amp_2026-09-17.md) 第 6.6 节 |
