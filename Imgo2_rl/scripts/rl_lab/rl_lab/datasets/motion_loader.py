...
from rl_lab.utils import utils
from rl_lab.datasets import pose3d
from rl_lab.datasets import motion_util
...
class AMPLoader:
    ...
    def reorder_from_pybullet_to_isaac(self, motion_data):
        ...
        fv_fl, fv_fr, fv_rl, fv_rr = np.split(
    ...
    def get_frame_at_time_batch(self, traj_idxs, times):
        ...
        all_frame_starts = torch.zeros(len(traj_idxs), frame_dim, device=self.device)
        all_frame_ends = torch.zeros(len(traj_idxs), frame_dim, device=self.device)
    ...
    def feed_forward_generator(self, num_mini_batch, mini_batch_size):
        ...
        for _ in range(num_mini_batch):
            if self.preload_transitions:
                ...
                s = self.get_amp_observation_from_full_frame_batch(self.preloaded_s[idxs])
                s_next = self.get_amp_observation_from_full_frame_batch(self.preloaded_s_next[idxs])
            else:
                s, s_next = [], []
                traj_idxs = self.weighted_traj_idx_sample_batch(mini_batch_size)
                times = self.traj_time_sample_batch(traj_idxs)
                for traj_idx, frame_time in zip(traj_idxs, times):
                    s.append(self.get_amp_observation_from_full_frame(self.get_full_frame_at_time(traj_idx, frame_time)))
                    s_next.append(
                        self.get_amp_observation_from_full_frame(
                            self.get_full_frame_at_time(traj_idx, frame_time + self.time_between_frames)))
    ...
    def observation_dim(self):
        ...
        return AMPLoader.JOINT_VEL_END_IDX - AMPLoader.JOINT_POSE_START_IDX + 1

    @staticmethod
    def get_amp_observation_from_full_frame(frame):
        return torch.cat([
    ...
    @staticmethod
    def get_amp_observation_from_full_frame_batch(frames):
        return torch.cat([
            frames[:, AMPLoader.JOINT_POSE_START_IDX:AMPLoader.JOINT_VEL_END_IDX],
            frames[:, AMPLoader.ROOT_POS_START_IDX + 2:AMPLoader.ROOT_POS_START_IDX + 3],
        ], dim=-1)
