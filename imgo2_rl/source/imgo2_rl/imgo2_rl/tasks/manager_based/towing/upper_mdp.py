...
class HierarchicalVelocityAction(ActionTerm):
    ...
    def _body_properties(self, asset, body_id, offset_world, *, effective_mass=None):
        inertias = asset.root_physx_view.get_inertias().to(self.device)[:, body_id]
        rotation = self._quat_to_matrix(asset.data.body_quat_w[:, body_id])
        inverse = world_inverse_inertia(
            (inertias[:, 0], inertias[:, 4], inertias[:, 8]), rotation)
    ...
    def _apply_towing_physics(self):
        ...
        masses = self._cart.root_physx_view.get_masses().to(self.device)
        inertias = self._cart.root_physx_view.get_inertias().to(self.device)
        wheel_inertia = sum(inertias[:, body_id, 4] for body_id in self._wheel_body_ids)
        cart_effective_mass = masses.sum(dim=1) + wheel_inertia / self.cfg.wheel_radius ** 2
